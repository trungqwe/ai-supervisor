...
3. **Durable Saga Operations (ADR-016 §27)**:
   ...
   - **`StopOperation` (`stop_operations`)**:
     ...
     - `STOP_OPERATION_RESOLVED` là audit `event_type` bổ sung cho đúng hai logical resolution `STOP_GENERATION_MISMATCH` và `STOP_EFFECT_UNPROVEN_TARGET_ALREADY_TERMINATED` (clarification TASK-P03-003C). Event không chứng minh termination và không clear quarantine; các resolution có event riêng giữ nguyên event đó.
