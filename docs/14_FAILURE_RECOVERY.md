...
| **REC-005** | Dirty Worktree Detected | Worktree contains uncommitted files or unsafe working tree state before task dispatch. | Supervisor Core | Fail closed; mark task attempt blocked/failed (`failure_reason = WORKTREE_DIRTY`); abort dispatch and escalate to human or upstream-owned resolution without mutating worktree (no `git stash`, `git clean`, checkout, reset, or supervisor-owned worktree mutation). |
