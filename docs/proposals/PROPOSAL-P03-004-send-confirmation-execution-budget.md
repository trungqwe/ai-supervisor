...
**EXEC-R1-001 — maintenance khi Run incomplete:** đề xuất `AcquireMaintenance(ctx, Pair, purpose)` là trusted scope độc quyền của **cùng host boundary**. Normal admission/serve vẫn đóng; host chặn Run mới và effect callers, drain/join cả caller đang chạy trên cùng DB/Pair qua các process, rồi mới cấp maintenance. Không mở SQLite transaction trong drain/AO call. Không lồng `Run` vào maintenance hoặc lồng normal `AcquireEffect` vào manual stop để tránh deadlock. Verified historical-policy authority chỉ cho Tx L; thiếu policy evidence phải dùng authority **khác** cho manual stop/reconciliation, không dựng budget. Đề xuất wrapper 3C nhận maintenance permit sống và chỉ caller thắng reservation mới được `/kill` một lần; wrapper/scope này chờ Supervisor duyệt. Tx L audit/CAS lỗi rollback; cancel/crash sau commit giữ durable row/intent, không replay effect. Release scope đúng một lần không mở admission; host phải Run lại thành công. Xem draft ADR §6.1 về actor, audit, replay và barrier tests. Fake provider không chứng minh host/cross-process exclusivity.
...
**EXEC-R1-002 — ba ranh giới:** draft ADR §5.1 có bảng đủ `active`, `waiting_input`, `blocked`, `idle`/handoff, `exited` chưa terminated, terminated, 404, identity/generation mismatch và outage/protocol error **trước Tx R, sau Tx R và trước kill**. Tx R commit intent + audit và double quarantine, giữ `RUNNING`/attempt mở. GET thứ hai của caller thắng sau Tx R có thể khác GET đầu; chỉ exact active/waiting_input cùng owner/permit còn hiệu lực mới gọi kill một lần. Với các observation khác, bỏ quyền effect, không xóa intent hoặc lấy quyền từ row. Đề xuất `RelinquishTimeoutEffect` CAS `IN_FLIGHT→STOP_REISSUE_REQUIRES_HUMAN` với `STOP_OPERATION_RESOLVED` audit, giữ wire stage/TaskState/quarantine; đây là mở rộng semantics chờ duyệt. Nếu không được duyệt hoặc CAS/audit lỗi, giữ unresolved intent để exclusive startup/human reconciliation. D11 terminal outcomes tiếp tục atomic closure/audit đã duyệt. GET mới không chứng minh AO giữ nguyên trạng thái đến thời điểm effect; cần barrier test thay observation giữa hai GET, cùng stale winner và lỗi audit.
...
**Supervisor cần duyệt:** DDL v5 và trigger/legacy policy; các literal `binding_basis`, event `EXECUTION_BUDGET_LEGACY_BOUND`, cause column; maintenance permit và authority manual stop khi Run incomplete; double quarantine tại Tx R; `RelinquishTimeoutEffect` và mở rộng `STOP_REISSUE_REQUIRES_HUMAN` cho caller sống bỏ quyền trước kill; bảng precedence chi tiết; phạm vi mở `internal/dispatch`, `internal/stop`, `internal/domain` và host admission interface. Chỉ sau approval mới reconcile canonical registry/specs và release contract revision 3. `3D-R1-005` còn OPEN.
