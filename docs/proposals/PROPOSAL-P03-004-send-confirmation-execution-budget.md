...
> **Status:** `DESIGN_APPROVED_AT_3babaa2`; contract revision 3 chưa release, implementation chưa được duyệt. Supervisor chấp thuận origin `dispatch_operations.confirmed_at`, maintenance độc quyền, Tx R dùng double quarantine của 3C và giữ intent unresolved khi caller bỏ effect. `3D-R1-005` vẫn OPEN.
...
**EXEC-R1-001 — maintenance khi Run incomplete (đã chấp thuận):** `AcquireMaintenance(ctx, Pair, purpose)` là trusted scope độc quyền của cùng host boundary. Normal admission/serve vẫn đóng; host chặn Run mới và effect callers, drain/join cả caller đang chạy trên cùng DB/Pair qua các process rồi mới cấp scope. Không mở SQLite transaction trong drain/AO call; không lồng Run hoặc normal `AcquireEffect` vào maintenance. Historical-policy binding và manual stop dùng verified authority riêng. Wrapper 3C chỉ cấp `/kill` một lần cho caller thắng reservation của exact current open `RUNNING` attempt. Legacy `DISPATCHED` không được ép `RUNNING`; thiếu evidence/đường reconciliation hợp lệ thì giữ fail-closed. Tx L audit/CAS lỗi rollback; cancel/crash giữ durable row/intent. Release scope không mở admission; phải Run lại. Xem ADR addendum chính thức §6.1. Fake provider không chứng minh host/cross-process exclusivity.
...
**EXEC-R1-002 — ba ranh giới (đã chấp thuận):** ADR addendum chính thức §5.1 có bảng `active`, `waiting_input`, `blocked`, `idle`/handoff, `exited` chưa terminated, terminated, 404, identity/generation mismatch và outage/protocol error **trước Tx R, sau Tx R và trước kill**. Tx R tái sử dụng atomic reservation 3C gồm intent, audit và double quarantine, giữ `RUNNING`/attempt mở. Guard trước effect nhận diện quarantine của chính winner, vẫn từ chối risk/owner mới. GET thứ hai có thể khác GET đầu; chỉ exact active/waiting_input cùng permit/ownership còn hiệu lực mới gọi kill một lần. Với observation khác, caller bỏ quyền và giữ `STOP_REQUESTED/IN_FLIGHT` cùng quarantine, không ghi resolution audit; chỉ exclusive startup/human reconciliation sau drain/join phân loại. 404/terminated/generation mismatch theo D11 atomic outcome đã duyệt. GET không chứng minh AO giữ nguyên trạng thái đến effect; barrier test đổi observation giữa hai GET.

**Phương án không chọn:** `RelinquishTimeoutEffect` chuyển live caller sang `STOP_REISSUE_REQUIRES_HUMAN` ngay khi bỏ quyền. Revision 3 giữ intent unresolved; không mở rộng `STOP_OPERATION_RESOLVED` hoặc tái cấp permit từ row.
...
Draft contract revision 3 giữ code base `583e700eb125a08cc6bd7d63b6b27a6f3d4cc527`; tiếp tục branch implementation chưa merge tại `00f64d0b26db2749eadd21b7307c98faa6b6280d`, giao ADR/contract artifact riêng, không reset các sửa R1-001..004. Whitelist cụ thể trong draft contract gồm file dispatch, stop, recovery, domain và Store hiện hữu cùng migration/test v5; thêm đúng `internal/store/migrations_v4_test.go` chỉ để đổi assertion **sau migrate-to-current**, giữ kiểm chứng v4, fixture lịch sử và rollback. `migrations_v3_test.go` dùng `migrateWithSchemas(...v3)` và assertion version 3 lịch sử nên không mở file đó. Callers `RecordSendConfirmed`/`ReserveStopOperation` trong recovery, dispatch, stop và Store tests đã nằm trong whitelist; không mở wildcard. Không sửa AOAdapter, P04 runner, daemon hay accepted ADR.

Test bắt buộc: before/equal/after deadline, `waiting_input` kéo dài, restart với policy mới, invalid/overflow timestamp, legacy có/thiếu evidence, confirm rollback, hai monitor, host quiescence/maintenance barrier không deadlock, crash sau intent, stop hiện hữu, hai GET đổi observation ngay trước effect, D11 cause/disposition tách biệt và audit rollback. Test fake provider chỉ chứng minh library contract.

**Quyết định Supervisor tại `3babaa2`:** chấp thuận thiết kế maintenance/authority riêng, v5 budget bền vững, Tx R/double quarantine và precedence; loại `RelinquishTimeoutEffect`. Addendum chính thức và canonical reconciliation ghi phạm vi supersede; candidate contract revision 3 chờ release audit riêng. `3D-R1-005` còn OPEN đến implementation audit; host quiescence/principal/startup wiring vẫn là dependency runtime.
