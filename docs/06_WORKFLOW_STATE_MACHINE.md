...
stateDiagram-v2
    ...
    RUNNING --> REPORT_READY: Worker Emits Report
    RUNNING --> FAILED: Process Crash / Timeout
...
| **`RUNNING`** | AO / Worker | Worker process active | `REPORT_READY`, `FAILED`, `BLOCKED` | Worker PID active in AO session. |
| **`REPORT_READY`** | Supervisor | Worker completes | `EVIDENCE_READY`, `FAILED` | Worker output conforms to `worker-report.schema.json`. |
