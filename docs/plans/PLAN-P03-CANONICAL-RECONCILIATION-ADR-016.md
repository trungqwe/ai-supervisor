...
- **Required Specification Synchronization**:
  ...
  2. *Durable Dispatch Saga Steps*: Reflect the 3-stage dispatch saga in `dispatch_operations`:
     ...
     - Pre-Send Admissibility Check: Evaluates `GetWorkerStatus` against strict whitelist (`idle`, `waiting_input`) matching bound `session_id` and `terminal_generation` immediately prior to Stage 2 (ADR-016 D7);
...
- **Required Specification Synchronization**:
  ...
  3. *Update REC-014 (Comprehensive Startup Recovery Sweep)*:
     - Replace informal scan with the synchronous recovery scanner from ADR-016 §19 (D13), preserving each exact stage predicate and outcome:
       ...
       4. Evaluate restart-stable `confirmation_deadline_at`:
           - For a prior `STOP_CALL_SUCCEEDED` with matching generation and `isTerminated == true`, if first observed before deadline (`now < confirmation_deadline_at`), advance to `STOP_TERMINATION_CONFIRMED` / `TERMINATION_CONFIRMED`. Diverge strictly by stop purpose:
             * For `purpose == 'RUNNING_ATTEMPT_STOP'` on a task in `RUNNING`: evaluate all six verifiable conditions of `WORKER_STOPPED_ALLOWED_IFF` (ADR-016 §17); if satisfied, set `task_attempts.ended_at = now`, record `recovery_disposition = 'WORKER_STOPPED'`, transition TaskState `RUNNING -> FAILED` (`failure_reason = WORKER_STOPPED`), and release attempt and Pair lane quarantine (`quarantine_state = 'CLEAN'`).
             * For `purpose == 'QUARANTINE_CLEANUP'` on a task already in terminal failure (`FAILED` or `HUMAN_REQUIRED`): physical Class A clearance (ADR-016 D5/D6/D11) clears attempt and session quarantine to `CLEAN`, but enforces ZERO TaskState transition, ZERO `ended_at` mutation, and does NOT evaluate `WORKER_STOPPED_ALLOWED_IFF` or write `recovery_disposition = 'WORKER_STOPPED'` (preserving the original diagnostic failure disposition).
  ...
  4. *Add Section 1.2: Double-Gated Quarantine Clearance Rules*:
     ...
     - Detail clearance classes:
       - **Class A (`PHYSICAL_EXECUTION_RESOLUTION`)**: Verify matching-generation physical termination using approved D11 stop evidence before deadline (`now < confirmation_deadline_at`). Strictly separate by stop purpose:
         * *Active Attempt Live Stop (`purpose == 'RUNNING_ATTEMPT_STOP'`)*: Governed strictly by `WORKER_STOPPED_ALLOWED_IFF` on a task in `RUNNING`. Sets `task_attempts.ended_at = now`, records `task_attempts.recovery_disposition = 'WORKER_STOPPED'`, transitions `RUNNING -> FAILED`, and clears attempt and Pair lane quarantine gates (`quarantine_state = 'CLEAN'`).
         * *Governed Quarantine Cleanup (`purpose == 'QUARANTINE_CLEANUP'`)*: Governed by ADR-016 D5/D6/D11 on an already-terminal task (`FAILED` or `HUMAN_REQUIRED`). Confirmed physical termination performs Class A clearance (`quarantine_state = 'CLEAN'`) with audit event `QUARANTINE_RESOLVED_PHYSICAL`, but strictly enforces ZERO TaskState transition, ZERO `ended_at` mutation, and does NOT evaluate `WORKER_STOPPED_ALLOWED_IFF` or record `recovery_disposition = 'WORKER_STOPPED'` (original failure cause preserved).
...
- **Resolution & Authorization**:
  ...
  - Active Gate transitions to **`READY_FOR_EXTERNAL_AUDIT`**.
