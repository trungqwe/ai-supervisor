...
# 5. Execution budget v5 provenance (Revision 3 released, implementation merged)

AO không cung cấp actual execution start, policy snapshot, deadline persistence, cross-process admission hoặc one-use effect ownership. Supervisor Store sở hữu `attempt_execution_budgets`, immutable v5 migration/trigger, atomic send confirmation và timeout stop cause. `internal/dispatch` capture injected policy trước send; `internal/recovery` monitor sau startup, không effect trong Run; `internal/stop` tái dùng 3C reservation/D11/D12. Host/bootstrap sở hữu trusted shared admission/quiescence và verified principal, gồm maintenance độc quyền khi Run incomplete. Tx R dùng double quarantine sẵn trong `ReserveStopOperation`, không thêm AO wire. Legacy manual stop chỉ exact open `RUNNING`; không tự mở `DISPATCHED`. `internal/store/migrations_v4_test.go` chỉ cần đổi assertion sau migrate-to-current, giữ test v4 lịch sử. Contract revision 3 đã RELEASED; implementation 3D EXTERNAL_AUDIT_APPROVED tại `7513f1b9f39fa459be15e0abc836c6258a610d8b` và code đã MERGED tại `35909d7b21cdfe6b9f5c309ea565c5f9f9fedeea`. Thư viện recovery và budget provenance đã hoàn tất ở library scope; các handoff dependencies runtime (`HOST_QUIESCENCE_INTEGRATION=OPEN`, verified host principal, `DESIGN_BLOCKER_3D_STARTUP_WIRING=PRESERVED`, `AUTOMATIC_RESTORE=DISABLED`) tiếp tục được giữ nguyên.


# 6. Host Bootstrap & Daemon Lifecycle Provenance (TASK-P03-004, ADR-017)
