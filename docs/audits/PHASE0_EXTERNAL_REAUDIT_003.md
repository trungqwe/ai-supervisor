...
3. **Mieruko exact headings** — `## Quy tac chung cho agent` does not exist in AGENTS.md; correct heading is `## Quyền truy cập`. `## Active Agents` is incomplete; correct heading is `## Active Agents and session recovery`. `## Tool catalog overview` does not exist; correct heading is `## MCP tools`. `createAdminRouter` signature was hand-reconstructed inaccurately.
