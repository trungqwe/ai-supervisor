# RELEASED TASK CONTRACT: TASK-P03-004
...
> **Status**: `RELEASED`
> **Authority**: Formulated pursuant to accepted `ADR-017` (ACCEPTED), approved `PROPOSAL-P03-005` (APPROVED), and `PLAN-HOST-INTEGRATION-DEPENDENCY.md`. Formally approved and released by External Supervisor release audit decision in `docs/audits/P03_TASK_004_RELEASE_AUDIT.md`.
> **Candidate Source Reference**: Approved candidate commit `fd2b516e7c801898d67b41083faf185c24ab4e87`, blob `2db164f1ab5336f61085d5b8d24b7903417ba225`. All JSON fields preserved verbatim.
> **Implementation Scope**: Authorized strictly within `allowed_scope` on branch `codex/p03-004`. Governance and release artifacts remain separate from implementation diff.
...
> [!IMPORTANT]
> **GOVERNANCE STATUS: RELEASED FOR IMPLEMENTATION**.
> This document is the authoritative released Task Contract for `TASK-P03-004`.
> Production code changes are authorized strictly within `allowed_scope` on branch `codex/p03-004` starting from base SHA `35909d7b21cdfe6b9f5c309ea565c5f9f9fedeea`.
> Governance and release artifacts remain separate from the implementation diff.
> Tách bạch hoàn toàn kiểm chứng: Mock AO test chạy tự động trong CI/harness không được suy diễn thành bằng chứng host principal thật hay Live AO proof.
