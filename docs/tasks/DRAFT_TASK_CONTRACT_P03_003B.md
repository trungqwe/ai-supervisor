...
> **Decision proposal**: `docs/proposals/PROPOSAL-P03-003-restore-and-pre-send-semantics.md` (`PENDING_REVIEW`); mục tiêu restore theo ADR D3 vẫn thuộc 3B. Proposal không cấp quyền thay ADR, scope hoặc release contract.
...
```json
{
  ...
  "acceptance_criteria": [
    ...
    "AC-3B-06: pre-send inspection follows the §4 outcome table: matching idle/waiting_input may advance; active/blocked/exited remain DISPATCH_BOUND with DISPATCHED, ended_at NULL and PRE_SEND_ADMISSIBILITY_REJECTED; matching termination uses WORKER_TERMINATION_UNKNOWN, generation mismatch uses STALE_EXECUTION_GENERATION, and each DISPATCHED -> FAILED closes the attempt with TASK_STATE_TRANSITION/error details atomically under D12. Only additional quarantine/observation semantics remain blocked; decision-gap rows never call /send.",
...
Điều kiện trước effect phải gồm Pair/session identity hiện hành, session `TERMINATED` nhưng có căn cứ restore, không có provisioning unresolved, chưa có `DISPATCH_BOUND` cần giữ snapshot generation cũ, và `GetWorkerStatus` cho thấy cùng session/generation. Automatic path cần Pair/attempt liên quan `CLEAN`; recovery trên quarantine cần quyền human và audit, không tự clearance nhờ restore. Phải có **bằng chứng bền vững trước effect** phân biệt restore với spawn/dispatch, đồng thời CAS giành quyền cho đúng một caller và chặn caller thứ hai kể cả sau crash. Sau HTTP 200 cùng response hợp lệ và quan sát generation mới, một transaction phải CAS đúng Pair/session/generation đã giữ quyền, cập nhật cùng WorkerSession và ghi audit; CAS, audit hoặc commit lỗi phải rollback toàn bộ local changes. Các điều kiện này là yêu cầu thiết kế, **chưa có cơ chế ghi nhận được chấp thuận để thực hiện**.
...
**DESIGN_BLOCKER_3B_RESTORE_PROTOCOL**: ADR-016, DDL §27 và registry hiện không định nghĩa restore intent, audit provenance, ownership/exclusion hoặc quy tắc reconcile ambiguous restore. `PROPOSAL-P03-003` khuyến nghị bổ sung restore operation bền vững và guard chung cho Pair; External Supervisor phải quyết định cơ chế, actor được phép tiếp tục sau ambiguity và bằng chứng upstream đủ để xác nhận mà không reissue. Mục tiêu restore theo D3 được giữ trong 3B. Đến khi addendum/contract được phê duyệt, 3B không gọi `ResumeWorker` tự động và không coi `CommitRestoredSession` là API đã được duyệt. Nếu quyết định cần sửa ADR/schema hoặc file ngoài whitelist, dừng để điều chỉnh contract trước release.

## 4. 3B-C1-002 — bảng kết quả pre-send

Bảng áp dụng tại `DISPATCH_BOUND` với `TaskState=DISPATCHED`, attempt đang mở, `ended_at=NULL`, `resolution_state=NULL` trước quan sát. `resolution_state` của dispatch không có token mới được phê duyệt cho các kết quả này: giữ `NULL` ở mọi hàng, kể cả khi task terminal, cho đến khi một quyết định khác được phê duyệt. ADR-016 §4/registry đã định nghĩa `WORKER_TERMINATION_UNKNOWN`; D7 đã chốt generation mismatch → `FAILED`/`STALE_EXECUTION_GENERATION`; D12 bắt buộc mọi `DISPATCHED -> FAILED` đóng attempt, ghi disposition, `TASK_STATE_TRANSITION` và error details trong cùng transaction. Các câu hỏi còn mở ở quarantine, identity/404/query observation và quy trình `exited` được đề xuất trong `PROPOSAL-P03-003`, chưa được duyệt. Ưu tiên kiểm tra identity, generation, `isTerminated`, rồi activity; mọi hàng bị từ chối đều cấm `/send`.
...
| Cùng identity/generation, `isTerminated=true` | `DISPATCH_BOUND / NULL` | `FAILED` (§22) | Set `now` (D12) | `WORKER_TERMINATION_UNKNOWN` (ADR §4/registry) | §22: không có hành động quarantine mới; giữ giá trị cũ, không tự clear | `TASK_STATE_TRANSITION` + error details trong transaction D12 | Không tiếp tục attempt; không suy diễn crash hay stop. Quy tắc quarantine bổ sung còn chờ proposal. |
| `terminalGeneration` khác bound snapshot | `DISPATCH_BOUND / NULL` | `FAILED` (D7) | Set `now` (D12) | `STALE_EXECUTION_GENERATION` (D7/registry) | Chính sách impose quarantine chưa được §22 chốt; giữ fail-closed, không clear | `PRE_SEND_ADMISSIBILITY_REJECTED` (D7) và `TASK_STATE_TRANSITION` + error details trong transaction D12 | Không tiếp tục attempt, không sửa snapshot; chỉ chính sách quarantine bổ sung còn chờ proposal. |
...
Với attempt còn mở, coordinator nạp lại **đúng** `dispatch_operations.attempt_id` và TaskAttempt/current_attempt đang bind, đối chiếu Pair/task/contract/session/generation rồi query lại; không gọi `PrepareBoundDispatch` lần nữa, không tạo attempt/operation mới. Chỉ một transaction `RecordSendRequested` có thể thắng CAS `DISPATCH_BOUND -> SEND_REQUESTED`. Nếu đã terminal hoặc generation đổi, đường cùng attempt không còn hợp lệ. Trước release cần External Supervisor chốt **chỉ** các ô bổ sung đánh dấu khoảng trống/đề xuất; các nghĩa vụ §4/D7/D12 nêu trên đã có hiệu lực. Mặc định thực thi fail-closed, không ghi token hoặc transition chưa được duyệt.

## 5. Failure boundaries của `/send`

| Boundary / observation | Durable state và transaction containment | Ownership phục hồi |
|---|---|---|
| Guard, query hoặc DB/audit lỗi **trước khi** `RecordSendRequested` commit | Chưa gọi `/send`. Nếu chỉ transaction này rollback và không có writer khác, còn `DISPATCH_BOUND / NULL`, `DISPATCHED`, attempt mở, `ended_at=NULL`; CAS conflict phải đọc lại durable state. | Coordinator 3B trong phiên hiện tại; T4 startup recheck thuộc 3D. |
| `SEND_REQUESTED` và `DISPATCH_SEND_REQUESTED` đã commit; transport error, caller context canceled, process crash trong call hoặc response validation failure (kể cả HTTP 200 thiếu/sai identity) | Intent bền vững nhưng delivery chưa được xác nhận; không được đảo về `DISPATCH_BOUND`, blind resend hoặc giải phóng lane. Transaction D5 ghi `DISPATCHED -> FAILED -> HUMAN_REQUIRED`, đóng attempt, `UNCERTAIN_DELIVERY_CRASH`, cả hai `QUARANTINED`, audit `UNCERTAIN_DELIVERY_QUARANTINE_IMPOSED`. Nếu containment fail, đọc lại durable state: rollback thuần giữ `SEND_REQUESTED`, CAS thua có thể đã có writer khác xác nhận/resolve. | Coordinator 3B xử lý lỗi đã thấy; 3D scanner xử lý tồn đọng T5 sau crash/rollback, clearance theo D5–D6/3C. |
| AO trả HTTP 200 response hợp lệ nhưng `RecordSendConfirmed` CAS/audit/commit thất bại | AO có thể đã nhận write; không gọi `/send` lại. Đọc lại durable operation/attempt/audit: rollback thuần giữ `SEND_REQUESTED`, `confirmed_at=NULL`, `DISPATCHED`; CAS thua có thể là writer khác đã `SEND_CONFIRMED` hoặc containment đã thắng. Không gán mặc định một state, không suy diễn `RUNNING`. | Coordinator 3B phân biệt stale caller với rollback thật; tồn đọng/chưa đọc được giao 3D. |
