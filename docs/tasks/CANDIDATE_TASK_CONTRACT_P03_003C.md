...
| Q → exact terminated đúng deadline | `STOP_TERMINATION_CONFIRMED` / `TERMINATION_CONFIRMED` | Giữ `FAILED/HUMAN_REQUIRED` / giữ | Giữ disposition lịch sử | Linked: WS/A giữ; unlinked: D6 clear đúng lineage nếu đủ proof, WS chỉ sau mọi risk | `STOP_OPERATION_CONFIRMED`; stop evidence Tx S2, unlinked D6 theo lineage, linked Tx D rồi D6 | Không mở lane trước clearance |
| M → exact terminated đúng deadline | `STOP_TERMINATION_CONFIRMED` / `TERMINATION_CONFIRMED` | Không task/attempt / N/A | N/A | Linked: WS và mọi A cũ giữ; unlinked: D6 chỉ WS sau mọi lineage | `STOP_OPERATION_CONFIRMED`; stop evidence Tx S2, D6 riêng theo basis | New-generation proof không clear old A |
