...
```json
{
  ...
  "constraints": [
    ...
    "Only exact current open RUNNING/SEND_CONFIRMED attempt with matching Pair/contract/session/generation, a valid immutable budget and no stop/risk may reserve a new RUNNING_ATTEMPT_STOP at now >= execution_deadline_at. Apply draft addendum section 5.1 pre-Tx R/post-Tx R/pre-kill matrix, including fresh second GET and stop-owned generic poller guard. Only the Tx R winner's live scoped permit may issue one /kill; persisted intent never grants effect. Relinquishment semantics are PENDING DESIGN APPROVAL. No TaskState edge or AO wire change.",
    "PENDING DESIGN APPROVAL: when startup Run is incomplete for a legacy SEND_CONFIRMED row without budget, normal admission/serve stays closed. Trusted host AcquireMaintenance must exclude Run, deny new effect callers and drain/join conflicting callers across processes before operator historical binding or manual reconciliation; release does not reopen admission and Run must complete again. Historical policy binding and manual stop require distinct verified authority; missing evidence cannot create a budget. Do not nest normal AcquireEffect inside maintenance or call Run while holding maintenance.",
    "PENDING DESIGN APPROVAL: Tx R atomically records timeout stop intent, STOP_OPERATION_REQUESTED audit and WorkerSession/current-attempt quarantine while RUNNING remains open. After Tx R, blocked/idle/exited/mismatch/error observations do not authorize /kill. Proposed RelinquishTimeoutEffect uses exact CAS and STOP_OPERATION_RESOLVED audit to set STOP_REISSUE_REQUIRES_HUMAN while retaining stage, TaskState and quarantine; without approval or on CAS/audit failure, keep IN_FLIGHT for exclusive startup/human reconciliation. Never delete intent or infer effect ownership from a row.",
  ...
  "acceptance_criteria": [
    ...
    "AC-3D-18: legacy active SEND_CONFIRMED missing budget keeps startup incomplete and normal admission/serve closed. Trusted maintenance excludes Run and drains/joins all conflicting callers before separate verified historical-policy binding or manual stop/reconciliation authority. Historical confirmed_at and policy evidence atomically insert LEGACY_OPERATOR_VERIFIED budget and EXECUTION_BUDGET_LEGACY_BOUND audit; absent/changed evidence, CAS loser or audit failure leaves it blocked. Cancellation/crash/release do not open admission; only a subsequent complete Run may do so. No now/started_at/current-policy fabrication or nested admission deadlock.",
    "AC-3D-19: timeout cause survives crash and is emitted as TASK_STATE_TRANSITION failure_reason=TIMEOUT when 3C D11/D12 closes a live attempt; recovery_disposition, stop resolution, quarantine and audit retain exact D11 outcome in the same transaction. Pre-Tx R/post-Tx R/pre-kill tests cover active, waiting_input, blocked, idle/handoff, exited-alive, terminated, 404, identity/generation mismatch and outage/protocol error. A deterministic barrier changes observation between two GETs: only a still exact active/waiting_input Tx R winner with live permit may call /kill once; other branches retain or reconcile intent/quarantine without replay, with exact audit/CAS rollback. Confirmation deadline remains independent and strict."
  ],
  ...
  "required_evidence": [
    ...
    "pre_and_post_reservation_two_GET_barrier_effect_ownership_and_relinquishment_tests",
  ...
  "stop_conditions": [
    ...
    "Legacy historical policy evidence or trusted operator authority is absent; do not synthesize a deadline or open admission. Manual reconciliation requires a separately approved maintenance authority and a one-use 3C effect path.",
...
`internal/recovery/timeout_monitor.go` là runtime component riêng, không chạy trong startup `Run`; guarded `stop.Coordinator.StartTimeout` lấy host shared admission permit rồi dùng luồng reservation/effect 3C. `internal/dispatch/coordinator.go` capture policy trước send; `internal/store/dispatch_transactions.go` ghi budget cùng HTTP 200 confirmation. `internal/stop/coordinator.go` tiếp tục là đường duy nhất cấp one-use `/kill` permit; Store reservation và D11 giữ exact lineage, timeout cause và audit atomic. `internal/store/session_lifecycle.go` generic dispatch-stage API phải chịu guard v5, không lách budget. **Chưa được duyệt:** maintenance-exclusive wrapper/authority cho legacy không đủ policy; double quarantine tại Tx R; live caller relinquishment với `STOP_REISSUE_REQUIRES_HUMAN`. Draft ADR §5.1/§6.1 là test oracle thiết kế, chưa là quyền code.
