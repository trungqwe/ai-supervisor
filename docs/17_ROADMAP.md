...
| **`P03`** | AO Integration | Implement `AOAdapter` connecting Supervisor domain to AO REST daemon API with session lifecycle management and bounded observation. | AOAdapter public REST integration, session lifecycle/dispatch, lifecycle observation/state reconciliation integration, raw session workspace-file read transport (zero P04 WorkerReport semantic ingestion). | Automated session creation, dispatch, observation reconciliation, raw workspace file read, and teardown pass without P04 EvidenceCollector dependencies. |
