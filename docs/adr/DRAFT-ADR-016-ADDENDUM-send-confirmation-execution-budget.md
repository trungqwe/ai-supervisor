...
### 5.1. Ranh giới quan sát và quyền effect (EXEC-R1-002; đề xuất)
...
Trong bảng, **I** là Tx R đã commit `STOP_REQUESTED/IN_FLIGHT`, cause `TIMEOUT`, `STOP_OPERATION_REQUESTED` audit, session và exact attempt `QUARANTINED`, TaskState `RUNNING`, `ended_at=NULL`. Đây là quarantine ngăn effect khác, không là D11 closure. **L** là caller thắng Tx R còn giữ scoped effect permit trong stack frame. Khi L từ bỏ quyền, persisted I không cấp quyền cho bất kỳ caller/restart nào. Đề xuất API `RelinquishTimeoutEffect` dùng CAS exact stop/Pair/session/generation/attempt, old `STOP_REQUESTED/IN_FLIGHT`, authority của L và observation; ghi `STOP_REQUESTED/STOP_REISSUE_REQUIRES_HUMAN`, `resolved_at`, `STOP_OPERATION_RESOLVED` cùng transaction, giữ `RUNNING`, `ended_at=NULL`, double quarantine. Đây là **mở rộng semantics** của resolution đã duyệt cho live caller tự bỏ quyền trước effect, cần Supervisor phê duyệt; nếu không duyệt, giữ I unresolved và chỉ phân loại sau exclusive host quiescence, tuyệt đối không gọi `/kill` từ row. CAS/audit lỗi giữ I, lỗi được trả về, Pair vẫn khóa.
...
| `blocked`, exact | Nhánh blocked escalation hiện có, không reserve timeout. | L bỏ quyền, không kill theo timeout. | Đề xuất `RelinquishTimeoutEffect`; nếu chưa duyệt/tx lỗi giữ I. Blocked escalation chỉ được CAS sau stop reconciliation, không ghi đè owner. |
| `idle`, exact; handoff available/unavailable | Handoff/MISSED_ACTIVE_WINDOW hiện có xét trước, không reserve timeout. | L bỏ quyền; handoff không được biến intent thành effect permit. | Như blocked; handoff chỉ chạy sau giải quyết stop/risk, không xóa I để retry. |
| `exited`, `isTerminated=false` | Lifecycle `exited` hiện có: không suy physical termination, không reserve timeout. | L bỏ quyền, không kill theo timeout khi trạng thái đã đổi. | Như blocked; giữ quarantine tới reconciliation. |
...
| Session identity hoặc generation mismatch | Nhánh mismatch/containment hiện có, không reserve timeout. | Không kill. Generation mismatch dùng D11 `STOP_GENERATION_MISMATCH`; identity/protocol không được giả thành generation mismatch. | Generation mismatch: D11/D12 atomic closure/quarantine/`STOP_OPERATION_RESOLVED`. Identity sai: L bỏ quyền theo đề xuất, hoặc giữ I unresolved; không ghi physical proof. |
| Query outage hoặc response/protocol invalid | Fail-closed, không reserve, không tự ghi timeout stop. | Không kill; không biến lỗi GET thành 404/terminated/alive. | L bỏ quyền theo đề xuất chỉ khi có basis kiểm chứng hợp lệ; nếu không, giữ I `IN_FLIGHT`, request audit duy nhất, quarantine và human/startup reconciliation. Không tự retry effect. |

Sau Tx R, stop owner có ưu tiên trước generic poller và timeout monitor; generic observation CAS phải từ chối khi stop gắn exact attempt còn hiện hành. Trước `/kill`, L kiểm tra permit còn thuộc stack frame đang hoạt động và durable ownership chưa đổi; host quiescence phải **drain/join**, không thu hồi một permit rồi tự nhận caller đã kết thúc. Kiểm tra này không cấp quyền từ row và không hứa hẹn AO đứng yên giữa GET và effect. Nếu ownership/recheck lỗi, không phát effect, giữ I hoặc commit relinquishment bằng CAS; stale writer đọc lại winner. Không xóa intent để monitor khác reserve lại. Các outcome D11 hiện có giữ audit/TaskState boundary đã duyệt; `RelinquishTimeoutEffect` là phần **chưa phê duyệt** và phải nằm trong release design trước implementation.
...
### 6.1. Trusted legacy maintenance khi Run incomplete (EXEC-R1-001; đề xuất)
...
Nếu không có bằng chứng policy lịch sử, `MANUAL_STOP_RECONCILIATION` cần verified principal với **scope riêng** cho residual execution risk; không tạo budget. Đề xuất host-gated wrapper dùng đúng reservation/effect một lần của 3C cho `RUNNING_ATTEMPT_STOP`, rồi D11/D12, Class B/C và D6 theo authority/evidence hiện có. Wrapper phải nhận maintenance permit sống và kiểm tra exact Pair/attempt; không thể gọi normal `AcquireEffect` trong maintenance (sẽ tự deadlock). Mở đường wrapper và authority scope này là **semantics mới chờ Supervisor duyệt**; không dùng principal string hay Store row làm permit, không gọi `/kill` khi không chứng minh caller thắng intent mới. Nếu stop intent đã tồn tại, chỉ reconciliation đã duyệt, không reissue.
