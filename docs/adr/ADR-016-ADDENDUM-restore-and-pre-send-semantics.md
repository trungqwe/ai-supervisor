# ADR-016 Addendum: Restore ownership và dispatch pre-send resolution

> **Status**: EXTERNAL_APPROVED — External Supervisor phê duyệt thiết kế tại audited SHA 3807343f06f99c0e11460c74522a2efb3be19491; implementation chưa được kiểm chứng.
> **Authority**: External Supervisor phê duyệt thiết kế, DDL và transition của bản draft được audit; contract và implementation vẫn chờ phê duyệt riêng. Proposal lịch sử giữ nguyên.
...
> **Gate**: DESIGN_BLOCKER_3B_RESTORE_PROTOCOL=CLOSED_BY_APPROVED_ADDENDUM; TASK_P03_003B/3C/3D=NOT_RELEASED; P03_CODE=HELD_PENDING_TASK_P03_003B_CONTRACT_RELEASE; ACTIVE_GATE=TASK_P03_003B_CONTRACT_PLANNING; bảo lưu DESIGN_BLOCKER_3D_STARTUP_WIRING.

## Hiệu lực và quan hệ với ADR-016

Addendum này supersede **chỉ** ADR-016 D3/D4 về restore admission và ownership trước binding; D5–D6 về `DELIVERY_OUTCOME_UNKNOWN` và clearance theo từng lineage; D7/§22 về pre-send hold precedence/CAS; D11 về linked `PAIR_MAINTENANCE` không gắn attempt cho runtime generation mới; D13 về phân loại restore intent sau crash; §27 về schema v4 trong §2. Các điều khoản còn lại giữ nguyên, nhất là TaskState graph, immutable bound snapshot, invariant stop lineage 3A và HTTP 200 send confirmation.

Host/bootstrap integration sở hữu verified operator principal. 3B được phép triển khai trusted interface và test fail-closed; nếu chưa có verified host principal thì `/restore` và linked stop không được enable. Principal giả trong test thư viện không chứng minh host authentication. `AUTOMATIC_RESTORE=DISABLED`; 3C sở hữu stop/clearance, 3D sở hữu scanner/poller. Phê duyệt thiết kế không release 3B/3C/3D hoặc xác nhận implementation.

## 1. Authority và quyết định đã duyệt

Supervisor phê duyệt durable restore operation + Pair guard, chỉ operator-authorized restore trên pinned AO, `AUTOMATIC_RESTORE=DISABLED`, cấm retry `/restore` khi outcome không xác định, và semantics `DELIVERY_OUTCOME_UNKNOWN`: terminal cho dispatch operation, độc lập với quarantine clearance. DDL và transition được duyệt ở cấp thiết kế; migration và behavior implementation chưa được kiểm chứng.
...
**Attempt đang mở**: `QUARANTINE_CLEANUP` không hợp lệ theo D11; restore admission đã chặn attempt mở. Pre-send hold/quarantine phải tiếp tục cùng attempt theo §6 hoặc đóng đúng attempt bằng D12 + disposition/audit trước khi `ClaimRestoreCleanup`; không đóng attempt của task khác. `PAIR_MAINTENANCE` không gắn attempt được duyệt cho runtime generation mới dù còn *historical closed attempt*; đây là mở rộng D11 được duyệt ở cấp thiết kế, không lách lineage của attempt cũ. Trong toàn bộ `RECOVERY_CLAIMED`/`CLEANUP_CLAIMED`, dispatch, provisioning và restore mới tiếp tục bị chặn; chỉ linked stop API và resolution/clearance đúng thứ tự được đi qua. Stop coordinator 3C xử lý `/kill`, scanner 3D chỉ phân loại/đọc state; không dồn hai phần này vào 3B.
...
| Observation/hold | State và authority theo addendum | Cùng attempt hay đóng/cleanup |
|---|---|---|
...
| Generation mismatch | D7 `FAILED`/`STALE_EXECUTION_GENERATION`, D12 closure; double quarantine cùng transaction theo addendum. | Không sửa snapshot; Class A/B/C theo D6. |
...
Acceptance scenarios đã được duyệt ở cấp thiết kế, chưa phải kết quả test implementation:
...
| Linked stop target generation = closed attempt snapshot; target = new runtime generation; không có attempt | Old-match dùng `QUARANTINE_CLEANUP` với exact 3A lineage; new-gen không gắn old attempt, PAIR_MAINTENANCE có restore link/operator provenance theo addendum; no-attempt không tạo fake attempt. Stale/mismatched insert rollback. |
...
| Migration fresh/v3→v4, rerun, FK/unique/CHECK, linked-stop nullability, rollback audit/CAS | DDL và transaction invariant phải được kiểm chứng sau contract release, không tuyên bố PASS trong draft. |

Acceptance test sẽ dùng fake AO; phê duyệt thiết kế không chứng minh restore trên session thật. DDL §2 được duyệt nhưng chưa triển khai: nullable `stop_operations.restore_operation_id` cho cả linked cleanup có/không có `attempt_id`; exact lineage và basis từng đối tượng do Store CAS + audit details kiểm chứng, không nới 3A-R1-002. Đây là invariant phải được probe ở implementation sau phê duyệt, không phải bằng chứng migration đã chạy.

## 9. Scope và release gate sau phê duyệt

DDL, trusted boundary, risk scope, recovery claim, linked-stop CAS, pre-send hold/terminal dispositions và audit tokens được duyệt ở cấp thiết kế; implementation/host integration cần audit riêng. AUTOMATIC_RESTORE=DISABLED. Blocker thiết kế 3B đã đóng bởi addendum; điều này không enable runtime restore hoặc release 3B.

Canonical reconciliation được chuẩn bị riêng: đối chiếu `docs/04_ARCHITECTURE.md`, `05_DOMAIN_MODEL.md`, `06_WORKFLOW_STATE_MACHINE.md`, `12_UPSTREAM_INTEGRATION.md`, `14_FAILURE_RECOVERY.md`, `21_TRACEABILITY_MATRIX.md`, `22_MODULE_PROVENANCE.md`, `docs/phases/P03_AO_INTEGRATION.md`; cập nhật scope/JSON/AC của `docs/tasks/DRAFT_TASK_CONTRACT_P03_003B.md` và release bằng quyết định riêng. Contract 3B cần whitelist mới cho `internal/store/migrations.go`, migration tests, `internal/domain/session_lifecycle.go`, `internal/store/session_lifecycle.go`, `internal/store/dispatch.go`, `internal/store/restore_transactions.go` và tests; guard stop/clearance thuộc 3C, startup scanner thuộc 3D. Các generic Store mutation API hiện có cũng phải được guard hoặc đóng đối với P03; chỉ liệt kê file mới mà không xử lý đường cũ là không đạt acceptance. Accepted ADR-016 và proposal lịch sử giữ nguyên; tài liệu reconciliation cần External Supervisor audit trước khi release 3B.
