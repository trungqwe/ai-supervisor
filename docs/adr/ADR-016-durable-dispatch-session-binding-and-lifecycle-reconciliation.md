# ADR-016: Durable Worker Session Binding, Dispatch Saga, and Lifecycle Reconciliation (Revision 6)

> **Status**: ACCEPTED (EXTERNAL_APPROVED by External Supervisor Re-Audit 006, commit `40d51ff`)
> **Date**: 2026-09-22 (Revision 1: 2026-09-23, Revision 2: 2026-09-23, Revision 3: 2026-09-23, Revision 4: 2026-09-23, Revision 5: 2026-09-23, Revision 6: 2026-09-23)
...
> **Implementation Guard**: `TASK-P03-003` remains `NOT_RELEASED`, `P03_CODE = HELD_PENDING_CANONICAL_RECONCILIATION_AND_TASK_CONTRACT`.
...
Initial draft of ADR-016 (commit `928b0d1`) was audited in `docs/audits/P03_ADR_016_EXTERNAL_AUDIT.md` (findings `ADR16R1-001..009`). Revision 1 (commit `f5fadbe`) was re-audited in `docs/audits/P03_ADR_016_EXTERNAL_REAUDIT_001.md` (findings `ADR16R2-001..009`). Revision 2 (commit `11624f0`) was re-audited in `docs/audits/P03_ADR_016_EXTERNAL_REAUDIT_002.md` (findings `ADR16R3-001..010`). Revision 3 (commit `ff59939`) was re-audited in `docs/audits/P03_ADR_016_EXTERNAL_REAUDIT_003.md` (findings `ADR16R4-001..008`). Revision 4 (commit `dedcd93`) was re-audited in `docs/audits/P03_ADR_016_EXTERNAL_REAUDIT_004.md` (findings `ADR16R5-001..005` recorded, `ADR16R4-008` not closed). This document was revised under **ADR-016 Revision 5** (commit `68b3699`), which reconciled honest TOCTOU stop provenance, `STOP_CALL_OUTCOME_UNKNOWN` transport ambiguity, atomic attempt closure on `BLOCKED -> HUMAN_REQUIRED`, and nullable worktree path.

Following External Supervisor Re-Audit 005 (`docs/audits/P03_ADR_016_EXTERNAL_REAUDIT_005.md`), four critical literal safety and consistency findings were recorded:
1. `ADR16R6-001` (CRITICAL): The crash window between `RUNNING -> BLOCKED` (which leaves `ended_at = NULL`) and `BLOCKED -> HUMAN_REQUIRED` was unrecovered because D13 startup recovery only enumerated `DISPATCHED` and `RUNNING` tasks. Without startup escalation recovery for `BLOCKED` tasks with open attempts, a power loss would orphan the attempt indefinitely because the canonical state graph contains no `BLOCKED -> RUNNING` edge.
2. `ADR16R6-002` (HIGH): D12 defined atomic transactions only for terminal transitions (`DISPATCHED/RUNNING -> FAILED`) and non-closing transitions (`RUNNING -> BLOCKED`), omitting the atomic transaction specification for attempt-closing nonterminal workflow escalation (`BLOCKED -> HUMAN_REQUIRED`).
3. `ADR16R6-003` (HIGH): The standalone D11 safety invariant for `WORKER_STOPPED` was too weak (permitting recording on `isTerminated == true` alone), regressing from finding R3. Positive intentional-stop provenance requires durably proven prior kill acceptance (`STOP_CALL_SUCCEEDED`) with matching execution identity.
4. `ADR16R6-004` (HIGH): The D6 parenthetical for unresolved provisioning operations was logically reversed (`stage NOT IN ('PROVISION_REQUESTED', 'PROVISION_FAILED')` instead of `stage IN`).
5. `ADR16R4-008` & `ADR16R5-001` / `002`: Residual token drift in the Lifecycle Observation Matrix (which still contained undeclared `PRE_SEND_BLOCKED`) and live governance contradictions in `AGENTS.md` required complete elimination.

This document establishes **ADR-016 Revision 6**, executing this surgical, literal-safety reconciliation with ZERO production code, ZERO database migrations, and ZERO canonical specification mutation.
...
6. How to execute intentional stop operations with durable provenance, fail-closed reconciliation, and explicit handling of unavoidable session-scoped TOCTOU risk under the pinned public API.
...
| **D2** | **Formally Amend ADR-012 Section 10** | Decouples Pair-scoped sandbox provisioning from task-scoped prompt dispatch; `/send` remains strictly post-`DISPATCHED`. | Retain strict post-dispatch session spawn. | None directly. | None. | `EXTERNAL_APPROVED` |
| **D3** | **Fail-Closed Spawn Disposition with Explicit Session Exclusivity Guard (`CREATE_NEW_WORKER_SESSION_ALLOWED_IFF`) & Durable Provisioning Operations** | Pinned AO spawn exposes no caller correlation ID; pre-binding spawn crashes tracked durably in `pair_provisioning_operations`; spawn allowed ONLY if no current WorkerSession exists; unowned sessions cannot be proven owned and are not killed automatically. | Automatic orphan reaper; Blind spawn when WorkerSession exists; Ephemeral in-memory quarantine. | Creates table `pair_provisioning_operations` with partial unique index; locks Pair lane on unresolved crash. | None. | `EXTERNAL_APPROVED` |
| **D4** | **Dedicated `dispatch_operations` Table with 1:1 Attempt Cardinality & Candidate TaskAttempt Extensions** | Tracks `DISPATCH_BOUND -> SEND_REQUESTED -> SEND_CONFIRMED`; enforces `UNIQUE(attempt_id)` (`# ONE_TASK_ATTEMPT = ONE_DISPATCH_OPERATION`); correctly references `task_attempts(attempt_id)`; defines candidate snapshot and quarantine fields on `task_attempts`. | Referencing non-existent `task_attempts(id)`; Permitting multiple dispatch sagas per attempt. | Creates `dispatch_operations` table with `UNIQUE(attempt_id)`. | None (TaskState remains `DISPATCHED`). | `EXTERNAL_APPROVED` |
| **D5** | **Fail-Closed Terminal Failure (`DISPATCHED -> FAILED`) with Immediate Quarantine** | Pinned `/send` lacks delivery proof on crash; assuming failure and quarantining prevents split-brain execution; clearance requires Class A, Class B, or Class C resolution. | Blind resend; Silent continuation; Direct `DISPATCHED -> HUMAN_REQUIRED`. | Records `resolution_state` in dispatch operations and audit log; sets `quarantine_state = 'QUARANTINED'` on both session and attempt. | `DISPATCHED -> FAILED -> HUMAN_REQUIRED`. | `EXTERNAL_APPROVED` |
| **D6** | **Option D: Double-Gated Quarantine with Dedicated Attempt Quarantine State** | Provides defense-in-depth across Pair lane and task attempt levels; separates independent safety gate (`task_attempts.quarantine_state`) from diagnostic outcome (`recovery_disposition`). | Single-level lane check; Conflating diagnostic disposition with quarantine safety truth. | `worker_sessions.quarantine_state` and dedicated `task_attempts.quarantine_state`. | Blocks `FAILED -> READY` while quarantined. | `EXTERNAL_APPROVED` |
| **D7** | **Strict Whitelist Pre-Send Admissibility (`idle`, `waiting_input` only) via Approved Adapter Surface** | SessionGuard blocks writes on `blocked`/`exited`/`terminated`; `active` before send indicates foreign turn; `terminal_generation` is opaque string; inspects activity via approved `GetWorkerStatus`. | Permitting `active` before send; Raw transport calls. | Records rejection audit event. | Rejection halts dispatch or fails attempt. | `EXTERNAL_APPROVED` |
| **D8** | **Option C: Persist Ambiguity Disposition (`MISSED_ACTIVE_WINDOW`) and Handoff Downstream with Verified Pre-Transition Check** | Respects task boundaries; verifies handoff capability prior to `DISPATCHED -> RUNNING`; fails `DISPATCHED -> FAILED` if unavailable at checkpoint; later failures use `RUNNING -> FAILED`. | Transition order contradiction; Immediate fail-closed; Predefining "no modified files = FAILED". | `task_attempts.recovery_disposition = 'MISSED_ACTIVE_WINDOW'`. | `DISPATCHED -> RUNNING` (or `DISPATCHED -> FAILED` if handoff unavailable). | `EXTERNAL_APPROVED` |
| **D9** | **Preserve `RUNNING`, Emit Diagnostic Telemetry, Continue Polling** | `waiting_input` indicates worker is at empty prompt; does not indicate failure or permission blockage. | Transition to `BLOCKED`; Transition to `FAILED`. | Diagnostic audit telemetry only. | None (remains `RUNNING`). | `EXTERNAL_APPROVED` |
| **D10** | **Graph-Compatible Blocked Handling: Preserve `RUNNING` with `AO_BLOCKED_DECISION`; Close Attempt at `BLOCKED -> HUMAN_REQUIRED` (`AO_BLOCKED_ESCALATED`)** | Canonical graph has NO `BLOCKED -> RUNNING` edge; AO `blocked` observation keeps TaskState `RUNNING` with open attempt; formal escalation `BLOCKED -> HUMAN_REQUIRED` atomically closes attempt (`ended_at = now`, `AO_BLOCKED_ESCALATED`) under invariant `HUMAN_REQUIRED_WITH_PRIOR_EXECUTION MUST_NOT_RETAIN_RESUMABLE_OPEN_ATTEMPT`. | Claiming `BLOCKED -> RUNNING` resumption; Retaining resumable open attempt after formal `HUMAN_REQUIRED` escalation. | `task_attempts.recovery_disposition = 'AO_BLOCKED_DECISION'` (observation) / `'AO_BLOCKED_ESCALATED'` (escalation). | Observation keeps `RUNNING`; formal escalation executes `RUNNING -> BLOCKED -> HUMAN_REQUIRED`. | `EXTERNAL_APPROVED` |
| **D11** | **Purpose-Aware Stop Lifecycle with Generation Precheck, Restart-Stable Deadlines, Ambiguous Transport Handling & Preserved Provenance** | Pinned `/kill` lacks atomic generation fence (`PINNED_KILL_GENERATION_ATOMIC_FENCE = ABSENT`); stop operations bind explicit `purpose` (`RUNNING_ATTEMPT_STOP`, `QUARANTINE_CLEANUP`, `PAIR_MAINTENANCE`); `stage` preserves wire-effect facts; transport ambiguity recorded as `STOP_CALL_OUTCOME_UNKNOWN`; persists `confirmation_deadline_at`. | Claiming atomic kill fence; Erasing wire-effect stage provenance; Unconditional `RUNNING -> FAILED` for cleanup stops; Converting transport ambiguity to definite call failure. | Complete `stop_operations` table with `purpose`, `confirmation_deadline_at`, and `resolved_at`. | `PURPOSE_DEPENDENT` (`RUNNING -> FAILED` only for `RUNNING_ATTEMPT_STOP` on `RUNNING` tasks; ZERO transition for cleanup/maintenance). | `EXTERNAL_APPROVED` |
| **D12** | **Unified StateStore Atomic Terminal Transition (`AtomicTerminalTransition`) without Undeclared Columns** | Ensures crash consistency: TaskState CAS, attempt `ended_at`, and `recovery_disposition` committed in one SQLite transaction; full error details logged in `audit_events.details_json`; zero undeclared columns. | Referencing nonexistent `terminal_error` on `task_attempts`; Non-transactional terminal updates. | Atomically updates `tasks`, `task_attempts`, `audit_events`. | Executes terminal failure transitions. | `EXTERNAL_APPROVED` |
| **D13** | **Synchronous Startup Recovery Sweep Covering Provisioning, Dispatch, Purpose-Aware Stops, and Mandatory Blocked Attempt Escalation (`# BLOCKED_WITH_OPEN_CURRENT_ATTEMPT MANDATORY_ESCALATION_RECOVERY`)** | Enumerates `pair_provisioning_operations`, `dispatch_operations`, and in-flight `stop_operations`; prohibits automatic `/kill` reissue (`STOP_REISSUE_REQUIRES_HUMAN`); evaluates restart-stable `confirmation_deadline_at`; uses approved adapter methods. | Scanning `STOP_REQUESTED` only; Automatic reissue of kill without atomic fence; Resetting stop timeout on restart. | Recovery audit records. | Reconciles in-flight tasks and sagas. | `EXTERNAL_APPROVED` |
...
5. **Stop Resolution States (`stop_operations.resolution_state`)**:
   ...
   - `STOP_CALL_FAILED`: Kill HTTP call positively rejected by upstream daemon (HTTP 4xx/5xx).
   - `STOP_CALL_OUTCOME_UNKNOWN`: Outbound kill transport failed ambiguously (timeout / reset); delivery unproven; retry prohibited (Finding `ADR16R5-004`).
...
6. **TaskAttempt Recovery Dispositions (`task_attempts.recovery_disposition`)**:
   ...
   - `AO_WAITING_INPUT_OBSERVED`: Worker paused at prompt; turn incomplete (diagnostic telemetry).
   - `AO_BLOCKED_DECISION`: Worker blocked on tool approval or permission dialog (observation).
   - `AO_BLOCKED_ESCALATED`: Prior attempt closed upon formal workflow escalation from `BLOCKED` to `HUMAN_REQUIRED` (Finding `ADR16R5-003`).
   ...
   - `STOP_GENERATION_MISMATCH`: Upstream execution generation changed unexpectedly.
   - `STOP_CALL_OUTCOME_UNKNOWN`: Stop transport failed ambiguously; attempt failed closed with quarantine (Finding `ADR16R5-004`).
...
8. **Audit Event Types (`audit_events.event_type`)**:
   ...
   - `PAIR_SESSION_PROVISIONED`: Infrastructure session bound upon HTTP 201.
   - `PAIR_SESSION_PROVISION_FAILED`: Upstream spawn call failed or rejected.
   - `TASK_DISPATCH_BOUND`: Task contract allocated and attempt bound.
   - `DISPATCH_SEND_REQUESTED`: Pre-effect dispatch intent recorded prior to `/send`.
   - `DISPATCH_SEND_CONFIRMED`: Upstream write acceptance recorded.
   - `PRE_SEND_ADMISSIBILITY_REJECTED`: Pre-send inspection rejected dispatch due to non-whitelisted activity or generation mismatch (Finding `ADR16R4-008`).
   ...
   - `WORKER_BLOCKED_ON_DECISION`: Tool permission dialog observed in AO.
   - `WORKER_BLOCKED_ESCALATED`: Prior attempt closed upon formal escalation to `HUMAN_REQUIRED` (Finding `ADR16R5-003`).
   - `STOP_OPERATION_REQUESTED`: Kill signal intent recorded.
   - `STOP_OPERATION_CALL_SUCCEEDED`: Kill signal acceptance recorded.
   - `STOP_OPERATION_CALL_FAILED`: Kill signal call positively rejected by upstream.
   - `STOP_OPERATION_CALL_OUTCOME_UNKNOWN`: Kill signal transport failed ambiguously; outcome unknown (Finding `ADR16R5-004`).
...
### Auditability Impact
Emits distinct audit events: `PAIR_SESSION_PROVISIONED` for infrastructure, `TASK_DISPATCH_BOUND` for execution.
...
3. **Dispatch Guard (`PrepareDispatch`)**:
   ...
   - Target Pair has no unresolved provisioning operations (there must exist ZERO rows for the target Pair where `stage IN ('PROVISION_REQUESTED', 'PROVISION_FAILED')`, evaluated via `SELECT COUNT(*) FROM pair_provisioning_operations WHERE pair_id = ? AND stage IN ('PROVISION_REQUESTED', 'PROVISION_FAILED') == 0`; Finding `ADR16R6-004`).
...
### Auditability Impact
Emits `UNCERTAIN_DELIVERY_QUARANTINE_IMPOSED` and `QUARANTINE_RESOLVED_PHYSICAL` / `QUARANTINE_RESOLVED_ADMINISTRATIVE` audit events.
...
1. State Transition Ordering:
   ...
   - **If Handoff Capability is Unavailable at Pre-Transition**: The Supervisor fails closed using the legal canonical edge `DISPATCHED -> FAILED` with `recovery_disposition = 'MISSED_ACTIVE_WINDOW'` and emits audit event `MISSED_ACTIVE_WINDOW_HANDOFF_DOWNSTREAM` (recording unavailable downstream handoff in `details_json`).
...
2. The poller records diagnostic observation `AO_WAITING_INPUT_OBSERVED` in telemetry and updates `task_attempts.recovery_disposition` (this is diagnostic telemetry / recovery disposition, not an audit event) and continues standard monitoring.
...
### Persistence Impact
Diagnostic telemetry / recovery disposition updates only (no audit event emitted).
...
- Finding `ADR16R5-003`: Keeping `ended_at = NULL` across `BLOCKED -> HUMAN_REQUIRED` leaves execution attempts orphaned in an open state because `HUMAN_REQUIRED` cannot return to `RUNNING` and subsequent replanning allocates a brand new attempt.

### Proposal Revision-6 Recommendation
- Preserve `RUNNING` with `AO_BLOCKED_DECISION` disposition; one-way formal escalation to `BLOCKED -> HUMAN_REQUIRED` (`GIẢ ĐỊNH`).

### Selected PROPOSED_DECISION
**PROPOSED_DECISION: Preserve `TaskState = RUNNING` with `AO_BLOCKED_DECISION` Lifecycle Disposition; Atomically Close Attempt at `BLOCKED -> HUMAN_REQUIRED` (`AO_BLOCKED_ESCALATED`) under Invariant `HUMAN_REQUIRED_WITH_PRIOR_EXECUTION MUST_NOT_RETAIN_RESUMABLE_OPEN_ATTEMPT` (Findings `ADR16R1-003`, `ADR16R5-003`)**.
...
- `TaskAttempt.ended_at` remains `NULL` (the attempt remains open and active).
...
2. **Formal Workflow Escalation & Mandatory Attempt Closure**:
...
- **`RUNNING -> BLOCKED`**: Atomically transitions TaskState to `BLOCKED`. `TaskAttempt.ended_at` remains `NULL` while in `BLOCKED` because `BLOCKED` is an intermediate non-terminal escalation state awaiting resolution or escalation.
- **`BLOCKED -> HUMAN_REQUIRED`**: The canonical workflow graph defines NO `BLOCKED -> RUNNING` or `HUMAN_REQUIRED -> RUNNING` edge. From `HUMAN_REQUIRED`, the only allowed transitions are `HUMAN_REQUIRED -> DRAFT` or `HUMAN_REQUIRED -> CANCELLED`. Any future replanning (`DRAFT -> READY -> DISPATCHED`) invokes `PrepareDispatch`, which increments `current_attempt` and allocates a brand new `TaskAttempt`.
- Therefore, transitioning `BLOCKED -> HUMAN_REQUIRED` **atomically closes the prior execution attempt** via the atomic transaction defined in D12 (`AtomicAttemptClosureTransition`, Finding `ADR16R6-002`):
  1. `tasks.state` CAS update (`BLOCKED -> HUMAN_REQUIRED`)
  2. verify current matching open attempt (`ended_at IS NULL`)
  3. `task_attempts.ended_at = now`
  4. `task_attempts.recovery_disposition = 'AO_BLOCKED_ESCALATED'`
  5. append audit event `WORKER_BLOCKED_ESCALATED`
  6. commit single SQLite transaction (zero network calls inside)
- The attempt is now permanently historical and must NEVER resume.
- **Safety Invariant**: `HUMAN_REQUIRED_WITH_PRIOR_EXECUTION MUST_NOT_RETAIN_RESUMABLE_OPEN_ATTEMPT`.
- **Crash Window Protection (Finding `ADR16R6-001`)**: If a host crash occurs after `RUNNING -> BLOCKED` is committed but before `BLOCKED -> HUMAN_REQUIRED` is executed, the startup recovery sweep in D13 deterministically identifies the open attempt and executes this exact atomic transaction (`# BLOCKED_WITH_OPEN_CURRENT_ATTEMPT MANDATORY_ESCALATION_RECOVERY`).
- Under no circumstances does the Supervisor attempt a non-existent `BLOCKED -> RUNNING` transition.

### Rationale
Resolves Findings `ADR16R1-003` and `ADR16R5-003`. Strictly conforms to `internal/workflow/state_machine.go` where `BLOCKED` has no return edge to `RUNNING`. Closing the attempt upon formal `HUMAN_REQUIRED` escalation prevents execution history corruption, ensuring that open attempts are never orphaned or retained in a non-resumable state when subsequent replanning allocates a new attempt.
...
- **Immediate Termination on Blocked Observation**: Aborts executions that merely require brief operator confirmation; rejected.
- **Retaining Open Attempt Across `HUMAN_REQUIRED` Escalation**: Rejected under Finding `ADR16R5-003`; leaves execution attempts permanently open with `ended_at = NULL` even though they can never resume.
...
### Safety Invariant
1. The Supervisor never attempts a state transition that violates the canonical workflow state graph defined in `internal/workflow/state_machine.go`.
2. `HUMAN_REQUIRED_WITH_PRIOR_EXECUTION MUST_NOT_RETAIN_RESUMABLE_OPEN_ATTEMPT`: Any task attempt with prior execution MUST be closed (`ended_at != NULL`) upon reaching `HUMAN_REQUIRED`.

### Persistence Impact
`task_attempts.recovery_disposition = 'AO_BLOCKED_DECISION'` (during observation) / `'AO_BLOCKED_ESCALATED'` (upon formal human escalation). `task_attempts.ended_at` is set to `now` at `BLOCKED -> HUMAN_REQUIRED`.

### State-Machine Impact
TaskState remains `RUNNING` during observation; one-way canonical path `RUNNING -> BLOCKED -> HUMAN_REQUIRED` used for formal escalation, atomically closing the prior execution attempt.

### Restart Impact
Restart scanner preserves `AO_BLOCKED_DECISION` with open attempt during observation, or `AO_BLOCKED_ESCALATED` with closed attempt if already escalated to `HUMAN_REQUIRED`.

### Auditability Impact
Audit event `WORKER_BLOCKED_ON_DECISION` recorded on observation; `WORKER_BLOCKED_ESCALATED` recorded upon formal escalation to `HUMAN_REQUIRED`.
...
- Finding `ADR16R1-006`: Stop operations must track full task attempt and execution generation lineage to prevent untracked terminations.
- Finding `ADR16R3-004`: Prematurely recording `WORKER_STOPPED` when termination cannot be verified corrupts the historical audit log.
- Finding `ADR16R3-006`: HTTP 404 target absence does NOT prove affirmative termination and must NOT advance to `STOP_TERMINATION_CONFIRMED`.
- Finding `ADR16R4-003`: Stop operations serve multiple purposes (`RUNNING_ATTEMPT_STOP`, `QUARANTINE_CLEANUP`, `PAIR_MAINTENANCE`). TaskState transitions are legally valid ONLY for live stops (`RUNNING_ATTEMPT_STOP`).
- Finding `ADR16R4-004`: Overwriting `stage` with diagnostic outcomes destroys the physical wire-effect history.
- Finding `ADR16R4-007`: Re-evaluating timeout on restart without a persisted deadline resets the timer and violates bounded recovery.
- Finding `ADR16R5-004`: When `/kill` transport fails ambiguously (network timeout or connection reset after bytes were transmitted), delivery outcome is unknown. Because `/kill` lacks an idempotency key, transport ambiguity must NOT be classified as definite `STOP_CALL_FAILED`, and automatic retry is strictly PROHIBITED.
- Finding `ADR16R5-005`: Pinned `/kill` has an unavoidable session-scoped TOCTOU window; intentional stop cannot claim 'zero race', but must provide durable provenance and fail-closed handling.

### Proposal Revision-6 Recommendation
- Multi-stage stop saga with generation precheck, restart-stable timeout, and purpose-aware TaskState reconciliation (`GIẢ ĐỊNH`).

### Selected PROPOSED_DECISION
**PROPOSED_DECISION: Implement Purpose-Aware Stop Operation Lifecycle with Generation Precheck, Restart-Stable Deadlines, Ambiguous Transport Handling, and Provenance Preservation (Findings `ADR16R4-001`, `ADR16R4-003`, `ADR16R4-004`, `ADR16R4-007`, `ADR16R5-004`, `ADR16R5-005`)**.
...
- `STOP_CALL_FAILED`: Committed ONLY when an outbound `/kill` call positively returns/decodes as failure at wire/protocol level (e.g. HTTP 4xx/5xx response from AO daemon). Emits audit event `STOP_OPERATION_CALL_FAILED`.
- `STOP_TERMINATION_CONFIRMED`: Committed when a subsequent observation snapshot confirms `isTerminated == true` matching target generation following `STOP_CALL_SUCCEEDED`. Emits audit event `STOP_OPERATION_CONFIRMED`.
- `STOP_TARGET_ABSENT`: Committed when upstream returns HTTP 404 Not Found. Proves session absence, but does NOT prove physical process termination. Emits audit event `STOP_OPERATION_TARGET_ABSENT`.

2. **Ambiguous Outbound Transport Outcome (`STOP_CALL_OUTCOME_UNKNOWN`, Finding `ADR16R5-004`)**:
When the outbound HTTP request encounters a connection reset, network timeout, or socket drop after request bytes were transmitted, the Supervisor cannot prove whether upstream AO received, accepted, or rejected the `/kill` signal:
- Because pinned `/kill` lacks a caller idempotency key, transport error != definite failure (`STOP_CALL_FAILED`).
- `stage` remains `STOP_REQUESTED` (no positive HTTP outcome was received).
- `resolution_state` becomes `STOP_CALL_OUTCOME_UNKNOWN`.
- `resolved_at = now`.
- Automatic retry or reissue is strictly PROHIBITED.
- For `purpose = 'RUNNING_ATTEMPT_STOP'`: fail closed via `AtomicTerminalTransition` (`RUNNING -> FAILED`, reason: `STOP_CALL_OUTCOME_UNKNOWN`, `ended_at = now`, quarantine attempt and Pair lane).
- For `purpose = 'QUARANTINE_CLEANUP'`: ZERO TaskState transition, quarantine remains active, human reconciliation required.
- For `purpose = 'PAIR_MAINTENANCE'`: ZERO TaskState transition, Pair/session safety state remains unresolved.
- Emits audit event `STOP_OPERATION_CALL_OUTCOME_UNKNOWN`.

3. **Complete Authoritative Schema**:
```sql
...
4. **Stop Operation Purpose & TaskState Semantics (Finding `ADR16R4-003`)**:
...
5. **Generation Precheck & Automatic Reissue Prohibition (Findings `ADR16R4-001`, `ADR16R5-005`)**:
...
6. **Wire-Effect Stage Provenance Preservation (Finding `ADR16R4-004`)**:
- `stage` preserves the physical wire-effect fact. Subsequent timeouts, mismatches, or ambiguities MUST NOT rewrite `stage`.
...
- `STOP_REQUESTED` + transport failure: `stage` remains `STOP_REQUESTED`, `resolution_state = 'STOP_CALL_OUTCOME_UNKNOWN'`.
...
7. **Restart-Stable Stop Confirmation Deadline (Finding `ADR16R4-007`)**:
...
- On daemon restart during `STOP_CALL_SUCCEEDED`:
  - If `now < confirmation_deadline_at`: remain in `STOP_CALL_SUCCEEDED` and continue observation until the original deadline. If same-generation `isTerminated == true` is observed while `now < confirmation_deadline_at`, advance to `STOP_TERMINATION_CONFIRMED` (`resolution_state = 'TERMINATION_CONFIRMED'`, `recovery_disposition = 'WORKER_STOPPED'`).
  - If first observation after restart occurs at or after deadline (`now >= confirmation_deadline_at`): the lineage-bounded temporal window has expired without timely termination confirmation. Because pinned AO exposes no process exit timestamp, exit code, or OS signal (`PUBLIC_CRASH_EVIDENCE_AVAILABLE_IN_PINNED_BASELINE = NONE`), the control plane MUST NOT speculate whether process exit occurred before or after the deadline. Even if upstream reports `isTerminated == true`, recording `WORKER_STOPPED` is strictly PROHIBITED under `WORKER_STOPPED_ALLOWED_IFF`. Advance directly to `resolution_state = 'STOP_CONFIRMATION_TIMEOUT'`, `resolved_at = now` (stage remains `STOP_CALL_SUCCEEDED`, `recovery_disposition = 'STOP_CONFIRMATION_TIMEOUT'`).
  - The timeout is never reset or extended on restart.

8. **Stop Purpose & TaskState Matrix**:

| Stop Purpose | Current TaskState | Observed Result | Allowed Task Transition | Attempt `ended_at` Action | Quarantine Action |
|---|---|---|---|---|---|
...
| `RUNNING_ATTEMPT_STOP` | `RUNNING` | `STOP_GENERATION_MISMATCH` | `RUNNING -> FAILED` (disposition: `STOP_GENERATION_MISMATCH`) | Set `now` via `AtomicTerminalTransition` | Quarantine remains ACTIVE on attempt & Pair |
...
| `RUNNING_ATTEMPT_STOP` | `RUNNING` | `STOP_CALL_OUTCOME_UNKNOWN` | `RUNNING -> FAILED` (`STOP_CALL_OUTCOME_UNKNOWN`) | Set `now` via `AtomicTerminalTransition` | Quarantine remains ACTIVE on attempt & Pair |
| `RUNNING_ATTEMPT_STOP` | `RUNNING` | `STOP_CALL_FAILED` | `RUNNING -> FAILED` (`WORKER_TERMINATION_UNKNOWN`) | Set `now` via `AtomicTerminalTransition` | Quarantine remains ACTIVE on attempt & Pair |
...
| `QUARANTINE_CLEANUP` | `FAILED` / `HUMAN_REQUIRED` | `STOP_CALL_OUTCOME_UNKNOWN` | **NONE** (Zero transition) | **NO-OP** (already terminal) | Quarantine remains ACTIVE |
| `QUARANTINE_CLEANUP` | `FAILED` / `HUMAN_REQUIRED` | `STOP_CALL_FAILED` | **NONE** (Zero transition) | **NO-OP** (already terminal) | Quarantine remains ACTIVE |
...
| `STOP_REQUESTED` | Transport Timeout / Socket Drop | Transport failed after bytes sent | Whether kill was accepted | **NO** (retry prohibited; un-idempotent) | `STOP_REQUESTED` | `STOP_CALL_OUTCOME_UNKNOWN` | Purpose-dependent (`RUNNING -> FAILED` for live stop) | **NO** (Finding `ADR16R5-004`) |
...
| `STOP_CALL_SUCCEEDED` | Same Generation, Alive (`now < confirmation_deadline_at`) | Kill accepted upstream; process not yet exited within deadline | Termination | **NO** (kill accepted; observe bounded by deadline) | `STOP_CALL_SUCCEEDED` | `IN_FLIGHT` | Purpose-dependent (remains `RUNNING` until deadline) | **NO** (Finding `ADR16R4-007`) |
| `STOP_CALL_SUCCEEDED` | Same Generation, Alive (`now >= confirmation_deadline_at`) | Kill accepted upstream; confirmation deadline expired while target alive | Termination | **NO** (deadline expired) | `STOP_CALL_SUCCEEDED` | `STOP_CONFIRMATION_TIMEOUT` | Purpose-dependent (`RUNNING -> FAILED` for live stop) | **NO** (Finding `ADR16R4-007`) |
...
| `STOP_CALL_SUCCEEDED` | Same Generation, `isTerminated == true` observed before deadline (`now < confirmation_deadline_at`) | Kill accepted upstream (HTTP 200) AND same-generation `isTerminated == true` observed within confirmation deadline | Absolute exit causality (whether process exit was caused strictly by `/kill` vs concurrent autonomous exit or crash; pinned AO exposes no exit reason, signal, or reaper token) | **NO** (already dead) | `STOP_TERMINATION_CONFIRMED` | `TERMINATION_CONFIRMED` | Purpose-dependent (`RUNNING -> FAILED` for live stop) | **YES** (Live stop only, under verified operational correlation within deadline) |
| `STOP_CALL_SUCCEEDED` | Same Generation, `isTerminated == true` first observed at/after deadline (`now >= confirmation_deadline_at`) | Kill accepted upstream (HTTP 200); target currently dead; observation occurred at or after deadline | Exit timing relative to deadline (pinned AO exposes no exit timestamp; cannot speculate if exit occurred before deadline) | **NO** (already dead) | `STOP_CALL_SUCCEEDED` | `STOP_CONFIRMATION_TIMEOUT` | Purpose-dependent (`RUNNING -> FAILED` for live stop) | **NO** (Violates `WORKER_STOPPED_ALLOWED_IFF` condition 6; fail closed) |
...
### Rationale
Resolves Findings `ADR16R1-006`, `ADR16R4-001`, `ADR16R4-003`, `ADR16R4-004`, `ADR16R4-007`, `ADR16R5-004`, and `ADR16R5-005`. Binds every stop operation to its full task attempt, contract revision, and execution generation lineage, while honestly reflecting the absence of an atomic kill generation fence and modeling ambiguous transport outcomes.

### Rejected Alternatives
- **Reduced Schema Omitting Task/Attempt Keys**: Rejected under Finding `ADR16R1-006` as insufficient for generation-attributed provenance.
- **Synchronous Immediate Termination Assumption**: Overlooking asynchronous termination timing; rejected.
...
- **Speculating Process Exit Timestamp from Observation on/after Deadline**: Rejected; pinned AO exposes no exit timestamp, so the control plane cannot speculate whether exit occurred before or after deadline; termination observed at or after deadline cannot be attributed to the lineage-bounded temporal window and fails closed to `STOP_CONFIRMATION_TIMEOUT`.
...
- Generation mismatch (observed `terminal_generation != stop_operations.terminal_generation`):
  ...
  Quarantine remains ACTIVE on attempt & Pair.
- Confirmation deadline expired (`now >= confirmation_deadline_at`):
  Applies when observation occurs at or after the deadline without timely prior termination confirmation. Even if upstream returns `isTerminated == true` upon observation at `now >= confirmation_deadline_at`, because pinned AO exposes no process exit timestamp, exit timing cannot be speculated. Condition 6 of `WORKER_STOPPED_ALLOWED_IFF` is not met.
  record `stage` remains `STOP_CALL_SUCCEEDED`, `resolution_state = 'STOP_CONFIRMATION_TIMEOUT'`, `recovery_disposition = 'STOP_CONFIRMATION_TIMEOUT'`.
...
### State-Machine Impact
`PURPOSE_DEPENDENT`:
- `RUNNING_ATTEMPT_STOP` on a task in `RUNNING` drives terminal failure `RUNNING -> FAILED` (reason: `WORKER_STOPPED` if confirmed, or respective failure reason such as `STOP_CONFIRMATION_TIMEOUT`, `STOP_CALL_OUTCOME_UNKNOWN`, `SESSION_ABSENT`, `STOP_GENERATION_MISMATCH`).
- `QUARANTINE_CLEANUP` causes **ZERO** TaskState transition (task is already terminal: `FAILED` or `HUMAN_REQUIRED`).
- `PAIR_MAINTENANCE` causes **ZERO** TaskState transition (no task attempt associated).

### Restart Impact
Startup scanner reconciles all non-terminal stop operations (`resolution_state = 'IN_FLIGHT'`).
...
## 18. D12: Atomic StateStore Transitions, Attempt Closure & ended_at Ownership
...
- Finding `ADR16R6-002`: D12 must define the atomic transaction shape for attempt-closing nonterminal workflow escalation (`BLOCKED -> HUMAN_REQUIRED`) to achieve unified architectural consistency with D10 and D13.

### Proposal Revision-6 Recommendation
- Unified StateStore atomic transition architecture defining both `AtomicTerminalTransition` and `AtomicAttemptClosureTransition` (`GIẢ ĐỊNH`).

### Selected PROPOSED_DECISION
**PROPOSED_DECISION: Implement Unified StateStore Atomic Transition Architecture Defining Both `AtomicTerminalTransition` and `AtomicAttemptClosureTransition` without Undeclared Columns (Findings `ADR16R2-003`, `ADR16R5-003`, `ADR16R6-002`)**.
1. **Terminal Failure Transitions (`RUNNING -> FAILED`, `DISPATCHED -> FAILED`)**:
   - Atomically committed in a single SQLite transaction via `AtomicTerminalTransition`:
     ```text
...
2. **Attempt-Closing Nonterminal Workflow Escalation (`BLOCKED -> HUMAN_REQUIRED`)**:
   - `HUMAN_REQUIRED` is NOT a terminal TaskState (the task can subsequently transition to `DRAFT` or `CANCELLED`).
   - However, because the canonical state graph contains no `BLOCKED -> RUNNING` or `HUMAN_REQUIRED -> RUNNING` edge, transitioning to `HUMAN_REQUIRED` definitively concludes the execution of the prior attempt.
   - Atomically committed in a single SQLite transaction via `AtomicAttemptClosureTransition` (Finding `ADR16R6-002`):
     ```text
     1. tasks.state CAS update (expected_state: BLOCKED -> target_state: HUMAN_REQUIRED)
     2. verify current matching open attempt (ended_at IS NULL)
     3. task_attempts.ended_at = now
     4. task_attempts.recovery_disposition = 'AO_BLOCKED_ESCALATED'
     5. audit_events row append (WORKER_BLOCKED_ESCALATED)
     6. commit
     ```
   - Zero network operations inside transaction.
   - This identical transaction is executed during live workflow escalation (D10) and during startup crash recovery (D13) to recover the crash window identified in `ADR16R6-001`.
3. **Intermediate Nonterminal Transition (`RUNNING -> BLOCKED`)**:
   - Atomically committed: TaskState CAS update from `RUNNING` to `BLOCKED` + append audit event `WORKER_BLOCKED_ON_DECISION`.
   - `task_attempts.ended_at` remains `NULL` while in `BLOCKED` because `BLOCKED` is an intermediate escalation state awaiting resolution or formal human escalation.
4. **Decoupling Observation from State CAS**:
...
5. **Success Closure (`RUNNING -> REPORT_READY`)**:
   - Strictly NOT owned by TASK-P03-003. Owned exclusively by Phase P04 Evidence Collector upon report validation.
6. **Transaction Boundary Rule**: All upstream AO queries and network interactions occur *strictly before* opening SQLite transactions. Transactions perform only local SQLite reads/writes and commit immediately.

### Rationale
Ensures crash consistency without risking SQLite database lock contention or introducing spurious state machine transitions for read-only observations. Defining both terminal and non-terminal attempt-closing transactions closes the specification gap identified in Finding `ADR16R6-002`.
...
- **Calling `HUMAN_REQUIRED` a Terminal State**: Incorrect; `HUMAN_REQUIRED` is non-terminal, but attempt closure is mandatory.

### Safety Invariant
Zero network operations inside SQLite transaction blocks.

### Persistence Impact
Adds unified transition methods to `internal/store` API (`AtomicTerminalTransition`, `AtomicAttemptClosureTransition`).

### State-Machine Impact
Executes all terminal state machine failures and nonterminal attempt-closing workflow escalations.

### Restart Impact
Partial writes are impossible; SQLite rollbacks on crash leave state completely clean.

### Auditability Impact
Atomic pairing of state change, attempt closure, and audit event guaranteed.
...
### Source / Canonical Facts
- Power loss leaves in-flight tasks in `DISPATCHED`, `RUNNING`, or `BLOCKED` with open attempts (`ended_at IS NULL`).
...
- Finding `ADR16R6-001`: Crash between `RUNNING -> BLOCKED` (attempt open) and `BLOCKED -> HUMAN_REQUIRED` leaves open attempt orphaned because canonical graph lacks `BLOCKED -> RUNNING`. Startup recovery must mandate deterministic escalation to `HUMAN_REQUIRED`.

### Proposal Revision-6 Recommendation
- Synchronous startup recovery sweep covering all in-flight tasks, blocked attempts, and non-terminal operations with existing poll cadence (`GIẢ ĐỊNH`).

### Selected PROPOSED_DECISION
**PROPOSED_DECISION: Implement Synchronous Startup Recovery Sweep Covering Provisioning, Dispatch, Blocked Escalation, and Purpose-Aware Generation-Prechecked Stops via Approved Adapter Methods**.
1. **Startup Enumeration**:
   ...
   - Query all tasks where `tasks.state = 'BLOCKED'` AND the current matching TaskAttempt has `ended_at IS NULL` (Finding `ADR16R6-001`).
...
3. **Stop Operation Recovery Protocol (Governed by Stop Recovery Truth Table & Purpose Matrix)**:
   ...
   - `STOP_REQUESTED`:
     ...
     - If `isTerminated == true`: Kill call was never confirmed transmitted/accepted. Retain `stage = 'STOP_REQUESTED'`, set `resolution_state = 'STOP_EFFECT_UNPROVEN_TARGET_ALREADY_TERMINATED'`, `resolved_at = now`. If `purpose == 'RUNNING_ATTEMPT_STOP'` and task is `RUNNING`: transition task `-> FAILED` (reason: `WORKER_TERMINATION_UNKNOWN`). Do NOT record `WORKER_STOPPED` (Findings `ADR16R3-004`, `ADR16R6-003`). For cleanup/maintenance stops: ZERO TaskState transition.
     - If generation mismatch: Retain `stage = 'STOP_REQUESTED'`, set `resolution_state = 'STOP_GENERATION_MISMATCH'`, `resolved_at = now`. If `purpose == 'RUNNING_ATTEMPT_STOP'` and task is `RUNNING`: transition task `-> FAILED` (disposition: `STOP_GENERATION_MISMATCH`).
   ...
   - `STOP_CALL_SUCCEEDED`:
     ...
     - **Decision Order for Observed Upstream Status & Restart-Stable Deadline**:
       1. **HTTP 404 (Target Absent)**: Set `stage = 'STOP_TARGET_ABSENT'`, `resolution_state = 'STOP_TARGET_ABSENT'`, `resolved_at = now`. If `purpose == 'RUNNING_ATTEMPT_STOP'` and task is `RUNNING`: transition task `RUNNING -> FAILED` (reason: `SESSION_ABSENT`), quarantine remains ACTIVE on attempt & Pair.
       2. **Generation Mismatch**: Retain `stage = 'STOP_CALL_SUCCEEDED'`, set `resolution_state = 'STOP_GENERATION_MISMATCH'`, `resolved_at = now`. If `purpose == 'RUNNING_ATTEMPT_STOP'` and task is `RUNNING`: transition task `RUNNING -> FAILED` (disposition: `STOP_GENERATION_MISMATCH`), quarantine remains ACTIVE on attempt & Pair.
       3. **Same Generation and `isTerminated == true`**:
          Evaluate observation timestamp `now` against restart-stable deadline `confirmation_deadline_at`:
          - **Case 1: Observation within deadline (`now < confirmation_deadline_at`)**: All 6 verifiable conditions of `WORKER_STOPPED_ALLOWED_IFF` are met. Set `stage = 'STOP_TERMINATION_CONFIRMED'`, `resolution_state = 'TERMINATION_CONFIRMED'`, `resolved_at = now`. If `purpose == 'RUNNING_ATTEMPT_STOP'` and task is `RUNNING`: `ended_at = now`, record `WORKER_STOPPED` (under `WORKER_STOPPED_ALLOWED_IFF`, Finding `ADR16R6-003`), release attempt quarantine. For cleanup/maintenance: ZERO TaskState transition.
          - **Case 2: First observation at or after deadline (`now >= confirmation_deadline_at`)**: Condition 6 of `WORKER_STOPPED_ALLOWED_IFF` is NOT met (`now < confirmation_deadline_at` is false). Pinned AO wire contract exposes no process exit timestamp, exit code, or OS signal (`PUBLIC_CRASH_EVIDENCE_AVAILABLE_IN_PINNED_BASELINE = NONE`); the control plane MUST NOT speculate whether process exit occurred before or after the deadline. Under strict fail-closed handling, recording `WORKER_STOPPED` is strictly PROHIBITED. Retain `stage = 'STOP_CALL_SUCCEEDED'`, set `resolution_state = 'STOP_CONFIRMATION_TIMEOUT'`, `resolved_at = now`. If `purpose == 'RUNNING_ATTEMPT_STOP'` and task is `RUNNING`: transition task `RUNNING -> FAILED` (disposition: `STOP_CONFIRMATION_TIMEOUT`), quarantine remains ACTIVE on attempt & Pair.
       4. **Same Generation and Alive (`isTerminated == false`)**:
          Evaluate restart-stable deadline `confirmation_deadline_at`:
          - If `now < confirmation_deadline_at`: remain in `stage = 'STOP_CALL_SUCCEEDED'`, `resolution_state = 'IN_FLIGHT'`, and continue bounded observation until deadline.
          - If `now >= confirmation_deadline_at`: deadline expired without termination. Retain `stage = 'STOP_CALL_SUCCEEDED'`, set `resolution_state = 'STOP_CONFIRMATION_TIMEOUT'`, `resolved_at = now`. If `purpose == 'RUNNING_ATTEMPT_STOP'` and task is `RUNNING`: transition task `RUNNING -> FAILED` (disposition: `STOP_CONFIRMATION_TIMEOUT`), quarantine remains ACTIVE on attempt & Pair; do NOT log `WORKER_STOPPED` (Findings `ADR16R3-009`, `ADR16R4-007`).
4. **Blocked Escalation Crash Recovery Protocol (`# BLOCKED_WITH_OPEN_CURRENT_ATTEMPT MANDATORY_ESCALATION_RECOVERY`, Finding `ADR16R6-001`)**:
   - **Crash Window**: If a host crash occurs after `RUNNING -> BLOCKED` is committed (leaving `task_attempts.ended_at = NULL`) but before live `BLOCKED -> HUMAN_REQUIRED` escalation executes, the task restarts in `BLOCKED` with an open attempt.
   - **Absence of Return Edge**: The canonical state machine (`internal/workflow/state_machine.go`) defines NO `BLOCKED -> RUNNING` edge. That attempt can NEVER resume execution. Retaining `ended_at = NULL` would permanently orphan the attempt, violating invariant `HUMAN_REQUIRED_WITH_PRIOR_EXECUTION MUST_NOT_RETAIN_RESUMABLE_OPEN_ATTEMPT`.
   - **Mandatory Escalation Execution**: On startup, the scanner queries:
     ```sql
     SELECT t.task_id, a.attempt_id
     FROM tasks t
     JOIN task_attempts a ON t.task_id = a.task_id AND a.attempt_number = t.current_attempt
     WHERE t.state = 'BLOCKED' AND a.ended_at IS NULL;
     ```
   - For each matching task, the scanner immediately executes `AtomicAttemptClosureTransition` (D12):
     1. Atomically CAS `tasks.state` from `BLOCKED` to `HUMAN_REQUIRED`;
     2. Sets `task_attempts.ended_at = now`;
     3. Sets `task_attempts.recovery_disposition = 'AO_BLOCKED_ESCALATED'`;
     4. Appends audit event `WORKER_BLOCKED_ESCALATED`;
     5. Commits single SQLite transaction.
   - **Zero Network Calls Required**: This transition is a deterministic internal control-plane workflow escalation; no AO daemon calls are made.
   - **Store Ground-Truth Note**: Production P02 store recovery currently scans `DISPATCHED` tasks only. This D13 specification defines the required TASK-P03-003 store expansion; it is NOT claiming that current P02 code already recovers `BLOCKED`. Future replanning from `HUMAN_REQUIRED` (`HUMAN_REQUIRED -> DRAFT -> READY -> DISPATCHED`) invokes `PrepareDispatch` which increments `current_attempt` and allocates a brand new `TaskAttempt`.
5. **Dispatch & In-Flight Task Recovery**:
...
### Rationale
Resolves Findings `ADR16R1-007`, `ADR16R1-009`, and `ADR16R6-001`. Enumerating all non-terminal stop operations closes the crash window where kill was accepted but termination was unconfirmed. Recovering crashed blocked tasks closes the attempt-orphaning window without inventing illegal graph transitions. Using `SUPERVISOR_ACTIVITY_POLL_INTERVAL` leverages existing unhardcoded policy surfaces without inventing unapproved configuration parameters.
...
### Safety Invariant
1. No new task can be dispatched on a Pair until that Pair's in-flight task status has been reconciled against upstream reality.
2. `# BLOCKED_WITH_OPEN_CURRENT_ATTEMPT MANDATORY_ESCALATION_RECOVERY`: No task left in `BLOCKED` after a crash may retain an open attempt with `ended_at IS NULL`. All such tasks are deterministically escalated to `HUMAN_REQUIRED` with closed attempts upon startup.

### Persistence Impact
Recovery audit events logged upon startup.

### State-Machine Impact
Reconciles interrupted states to valid canonical destinations (`BLOCKED -> HUMAN_REQUIRED`, `DISPATCHED/RUNNING -> FAILED`).

### Restart Impact
Governs the complete restart recovery lifecycle of the control plane across all in-flight and interrupted tasks.

### Auditability Impact
Emits `STARTUP_RECOVERY_SWEEP_STARTED`, `STARTUP_RECOVERY_SWEEP_COMPLETED`, and `WORKER_BLOCKED_ESCALATED` audit events.
...
| `DISPATCHED` | `DISPATCH_BOUND` | `active` | Match | Foreign/lingering turn | Current task sent | None | Keep NULL | Emit `PRE_SEND_ADMISSIBILITY_REJECTED` (attempt open; no dispatch send) |
| `DISPATCHED` | `DISPATCH_BOUND` | `blocked` | Match | Dialog blocked | Current task sent | None | Keep NULL | Emit `PRE_SEND_ADMISSIBILITY_REJECTED` (attempt open; no dispatch send) |
| `DISPATCHED` | `DISPATCH_BOUND` | `exited` | Match | Process exit signal | Termination/crash | None | Keep NULL | Emit `PRE_SEND_ADMISSIBILITY_REJECTED` (attempt open; no dispatch send) |
| `DISPATCHED` | `DISPATCH_BOUND` | `isTerminated: true` | Match | Session dead | Cause of death | `-> FAILED` | Set `now` | None |
| `DISPATCHED` | `SEND_REQUESTED` | Any | Match/Mismatch | Delivery unknown | Delivery success | `-> FAILED` | Set `now` | `task_attempts.quarantine_state = 'QUARANTINED'`, `recovery_disposition = 'UNCERTAIN_DELIVERY_CRASH'` |
...
| `BLOCKED` | Formal escalation | N/A | Match | Human resolution required | Execution resumption | `-> HUMAN_REQUIRED` | Set `now` | `AO_BLOCKED_ESCALATED` |
...
| **T7a** | `RUNNING -> BLOCKED` committed, crash before `BLOCKED -> HUMAN_REQUIRED` | `tasks.state = 'BLOCKED'`, current attempt `ended_at IS NULL` | Task in `BLOCKED` with open attempt | Execute atomic nonterminal escalation `BLOCKED -> HUMAN_REQUIRED`: set `ended_at = now`, `recovery_disposition = 'AO_BLOCKED_ESCALATED'`, emit `WORKER_BLOCKED_ESCALATED` (`# BLOCKED_WITH_OPEN_CURRENT_ATTEMPT MANDATORY_ESCALATION_RECOVERY`, Finding `ADR16R6-001`) |
...
| **T8a** | `STOP_REQUESTED` committed, transport drop during `/kill` | Saga = `STOP_REQUESTED` | Delivery unknown | Reconcile via `GetWorkerStatus`; auto-reissue prohibited; advance to `resolution_state = 'STOP_CALL_OUTCOME_UNKNOWN'`, fail closed if live stop; if dead: `STOP_EFFECT_UNPROVEN_TARGET_ALREADY_TERMINATED`; if 404: `STOP_TARGET_ABSENT` |
| **T9** | `STOP_CALL_SUCCEEDED` committed, crash before confirm | Saga = `STOP_CALL_SUCCEEDED` | Kill accepted | Observe status; evaluate restart-stable `confirmation_deadline_at`. If `now < confirmation_deadline_at` and `isTerminated == true`: advance to `STOP_TERMINATION_CONFIRMED` (`TERMINATION_CONFIRMED`, `WORKER_STOPPED` if live stop). If first observed at/after deadline (`now >= confirmation_deadline_at`) or alive until deadline: advance to `resolution_state = 'STOP_CONFIRMATION_TIMEOUT'` (stage remains `STOP_CALL_SUCCEEDED`, `recovery_disposition = 'STOP_CONFIRMATION_TIMEOUT'`, `WORKER_STOPPED` strictly prohibited even if `isTerminated == true` as exit timing cannot be speculated). If 404: `STOP_TARGET_ABSENT`. If mismatch: `STOP_GENERATION_MISMATCH`. |
...
4. `AtomicTerminalTransition`: Single SQLite transaction committing terminal TaskState update (`old_state -> FAILED`), `task_attempts.ended_at = now`, `recovery_disposition`, error details in `audit_events.details_json`, and audit log.
5. `AtomicAttemptClosureTransition` (Finding `ADR16R6-002`): Single SQLite transaction committing nonterminal workflow escalation (`tasks.state` CAS `BLOCKED -> HUMAN_REQUIRED`), `task_attempts.ended_at = now`, `task_attempts.recovery_disposition = 'AO_BLOCKED_ESCALATED'`, and audit event `WORKER_BLOCKED_ESCALATED`.
6. Strict Boundary: No network call or AO interaction is ever permitted inside these transaction blocks.
...
- Truthfully models the absence of an atomic kill generation fence in pinned AO (`PINNED_KILL_GENERATION_ATOMIC_FENCE = ABSENT`), prohibiting automatic re-kill on restart and modeling ambiguous transport outcomes (`STOP_CALL_OUTCOME_UNKNOWN`).
...
- Kill operations have an unavoidable session-scoped TOCTOU window due to pinned upstream API limitations; the Supervisor bounds this risk through durable provenance, client generation prechecks, and restart reissue prohibitions.
...
Execution is halted awaiting canonical specification reconciliation planning under docs/24_CHANGE_GOVERNANCE.md.
