...
**Trusted caller boundary**: một Supervisor service nội bộ nhận principal/role đã được host xác thực và quyết định scope; chỉ service đó được insert authorization/risk audit hoặc gọi recovery/clearance. API worker, TaskContract parameters, AO response, `actor` string, event FK hay raw Store method không cấp quyền. Nếu host chưa cung cấp verified operator principal boundary, authorization issuance phải fail closed và contract không release; addendum không lập IAM/UI mới.

## 3. Pair guard, unresolved predicate và API ownership

`RESTORE_UNRESOLVED(pair) := EXISTS pair_restore_operations WHERE pair_id=? AND resolution_state<>'RESTORE_RESOLVED'`. Stage `RESTORE_CONFIRMED` vẫn khóa đến resolution. `DISPATCH_UNRESOLVED(pair)` chỉ bao gồm operation của **attempt đang mở/current** ở `DISPATCH_BOUND` hoặc `SEND_REQUESTED`; `SEND_REQUESTED` với `resolution_state IS NULL` luôn khóa. Operation `DISPATCH_BOUND` của attempt đã đóng là lịch sử, không khóa restore mãi; `SEND_REQUESTED` đã có terminal `DELIVERY_OUTCOME_UNKNOWN` khóa qua D6 quarantine, không qua predicate stage. Nếu dữ liệu có closed attempt nhưng `SEND_REQUESTED`/NULL, coi là inconsistent unresolved và khóa để audit, không âm thầm bỏ qua. `STOP_UNRESOLVED(pair)` bao gồm stop in-flight theo D11/D13 (stage/`resolution_state` chưa terminal), không chỉ `STOP_REQUESTED`.

```sql
-- Cùng transaction reservation; open DISPATCH_BOUND dù current_attempt lệch vẫn khóa để audit.
SELECT EXISTS (
  SELECT 1 FROM dispatch_operations d
  JOIN task_attempts a ON a.attempt_id = d.attempt_id
  WHERE d.pair_id = :pair_id AND
    ((d.stage = 'DISPATCH_BOUND' AND a.ended_at IS NULL) OR
     (d.stage = 'SEND_REQUESTED' AND d.resolution_state IS NULL))
);
```
...
|---|---|---|
| `ReservePairRestore` | Exact Pair/session/generation `TERMINATED`, no `DISPATCH_UNRESOLVED`, provisioning unresolved, stop unresolved hoặc restore unresolved; quarantine cũ phải được giải theo D6 trước restore mới. Consume exact one-shot authorization, intent/audit atomic. | 3B |
...
2. **Unknown outcome nhưng có physical evidence**: operator yêu cầu cleanup của *session hiện quan sát*, không suy từ generation khác. `ClaimRestoreCleanup` transaction CAS `RESTORE_RECOVERY_CLAIMED → RESTORE_CLEANUP_CLAIMED`, insert **một** `stop_operations` với `restore_operation_id`, exact Pair/session/observed generation, purpose `QUARANTINE_CLEANUP` chỉ nếu attempt liên quan **đã đóng** ở `FAILED/HUMAN_REQUIRED`; purpose `PAIR_MAINTENANCE` nếu **không có attempt**. Verified operator phải chấp nhận bằng audit details rủi ro `/kill` chỉ session-scoped, không có atomic generation fence; `PAIR_RESTORE_CLEANUP_CLAIMED` và `STOP_OPERATION_REQUESTED` cùng transaction, sau commit 3C mới gọi pinned `/kill` theo D11. Nếu generation đổi, không chứng minh stop an toàn: không call, chuyển Class C. Stop HTTP 200 + within-deadline `isTerminated=true` đúng generation cho Class A; Tx D CAS restore `CLEANUP_CLAIMED → RESOLVED`, basis `PHYSICAL_EXECUTION_RESOLUTION`, audit exact stop evidence. Stop `404`, timeout, transport unknown hoặc mismatch không được ghi physical/`WORKER_STOPPED`; operator dùng Class B/C nếu chấp nhận rủi ro. Không tự reissue `/kill`; stop in-flight giữ restore lock.
...
4. **D6 clearance**: chỉ sau Tx D, một transaction riêng CAS exact WorkerSession/attempt quarantine + lineage, chọn Class A physical hoặc Class B/C administrative evidence, append `QUARANTINE_RESOLVED_PHYSICAL` hoặc `QUARANTINE_RESOLVED_ADMINISTRATIVE`. Không có attempt thì chỉ WorkerSession được clear; không tạo fake attempt. Nếu clearance rollback, operation vẫn `RESOLVED` nhưng quarantine vẫn khóa dispatch/provisioning/restore mới. Nếu clearance thắng trước Tx D thì phải bị từ chối. `DELIVERY_OUTCOME_UNKNOWN` trên dispatch operation giữ nguyên xuyên suốt.

**Attempt đang mở**: `QUARANTINE_CLEANUP` không hợp lệ theo D11. Pre-send hold/quarantine phải tiếp tục cùng attempt theo §6 hoặc đóng đúng attempt bằng D12 + disposition/audit trước khi `ClaimRestoreCleanup`; không đóng attempt của task khác. `PAIR_MAINTENANCE` chỉ khi thật sự không có attempt liên quan. Trong toàn bộ `RECOVERY_CLAIMED`/`CLEANUP_CLAIMED`, dispatch, provisioning và restore mới tiếp tục bị chặn; chỉ linked stop API và resolution/clearance đúng thứ tự được đi qua. Stop coordinator 3C xử lý `/kill`, scanner 3D chỉ phân loại/đọc state; không dồn hai phần này vào 3B.
...
Acceptance scenarios trước release: (1) hai restore callers và cross-type dispatch/provisioning/stop tranh Pair; (2) authorization exact tuple/scope, one-shot replay kể cả generation không đổi, untrusted actor bị từ chối; (3) `REQUESTED`, `CONFIRMED`, `OUTCOME_UNKNOWN` sau crash; invalid/lost response, 404/409, HTTP 200 rồi DB rollback; (4) linked stop Class A/B/C, stop mismatch/timeout, restore lock không mở sớm; (5) attempt mở/đóng/không có attempt, đúng stop purpose và zero fake attempt; (6) pre-send hold recovery cùng attempt, operator abandonment/cleanup, quarantine không auto clear; (7) D5 writer race, late confirmation, replay, clearance giữ `DELIVERY_OUTCOME_UNKNOWN`; (8) migration fresh/v3→v4, rerun, FK/unique/CHECK và rollback audit/CAS. Test dùng fake AO; **không gọi restore trên session thật** trong lượt tài liệu này.
